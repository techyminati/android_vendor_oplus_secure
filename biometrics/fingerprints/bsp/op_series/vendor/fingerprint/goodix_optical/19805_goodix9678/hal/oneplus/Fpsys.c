...
#define FP_SENDOR_TYPE_PATH  "/sys/devices/platform/soc/soc:fingerprint_detect/sensor_version"
...
int32_t fp_get_sensor_type_from_path(void){
    ...
    int32_t count = 0;
    char value[16] = {0};
    fd = open(FP_SENDOR_TYPE_PATH, O_RDONLY);
    if (fd < 0) {
        LOG_E(LOG_TAG, "open %s fail.",FP_SENDOR_TYPE_PATH);
        id = -1;
    } else {
        count = read(fd, value, sizeof(value));
        if (count <= 0) {
            LOG_E(LOG_TAG, "read %s fail.",FP_SENDOR_TYPE_PATH);
            id = -1;
        } else {
            id = atoi(value);
            LOG_D(LOG_TAG, "fp sensor version is: 0x%x", id);
        }
        close(fd);
    }
    return id;
