 ...
 */
    ...
    gf_error_t CustomizedFingerprintCore::prepareEnrollRequest() {
        ...
        if (cDelmarSensor->isSensorVaild < 0) {
            LOG_E(LOG_TAG, "[%s] The sensor is invaild(%d)!", __func__, cDelmarSensor->isSensorVaild);
            return GF_ERROR_SENSOR_NOT_AVAILABLE;
        }
...
/*
    ...
    gf_error_t CustomizedFingerprintCore::send_auth_dcsmsg(AuthenticateContext* context, bool notified) {
        ...
        if (mNotify != nullptr) {
            ...
            message.data.auth_dcsmsg.chip_ic = 9638;//invalid
