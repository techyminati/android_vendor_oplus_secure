...
namespace goodix {
    ...
    gf_error_t Device::sendKeyEvent(gf_nav_code_t code)
    {
        gf_error_t err = GF_SUCCESS;
        FUNC_ENTER();
    ...
    void Device::keyTimerThread(union sigval v)
    {
        VOID_FUNC_ENTER();
        gf_error_t err = GF_SUCCESS;
        ...
        do
        {
            ...
            Device* device = static_cast<Device*>(v.sival_ptr);
    ...
    void Device::startKeyTimer(uint32_t timeoutSec)
    {
        VOID_FUNC_ENTER();
        do
        {
            ...
            if (NULL == mKeyTimer)
            {
                mKeyTimer = Timer::createTimer((timer_thread_t) keyTimerThread, this);
                if (NULL == mKeyTimer)
                {
            ...
            err = mKeyTimer->set(timeoutSec, 0, timeoutSec, 0);
            if (err != GF_SUCCESS)
            {
    ...
    gf_error_t Device::sendSleepEvent()
    {
        gf_error_t err = GF_SUCCESS;
        FUNC_ENTER();

        if (nullptr != mContext && nullptr != mContext->mSensor && (false == mContext->mFingerprintCore->isAuthDownDetected()) && true == mContext->isStopCaptureDone)
        {
    ...
    void Device::sleepTimerThread(union sigval v)
    {
        VOID_FUNC_ENTER();
        gf_error_t err = GF_SUCCESS;
    ...
    void Device::startSleepTimer(uint32_t timeoutSec)
    {
        VOID_FUNC_ENTER();
        ...
        do
        {
            ...
            if (NULL == mSleepTimer)
            {
                mSleepTimer = Timer::createTimer((timer_thread_t) sleepTimerThread, this);
                if (NULL == mSleepTimer)
                {
            ...
            err = mSleepTimer->set(timeoutSec, 0, timeoutSec, 0);
            if (err != GF_SUCCESS)
            {
    ...
    void Device::handleNetlinkReceivedData(void* data, uint32_t len) {
        ...
        value = *((uint8_t*)data);
        LOG_V(LOG_TAG, "[%s] GF netlink thread received msg %u.", __func__, value);
    ...
    gf_error_t Device::hold_wakelock(gf_wakelock_status mode)
    {
        ...
        do
        {
            if (mFD < 0)
            {
                LOG_E(LOG_TAG, "[%s], no device", __func__);
                err = GF_ERROR_HAL_FILE_DESCRIPTION_NULL;
                break;
            }
