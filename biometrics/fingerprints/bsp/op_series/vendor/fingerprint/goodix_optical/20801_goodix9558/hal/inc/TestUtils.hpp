...
#define DARK_PIXEL_UPPER_RIGHT_CORNER  2
    ...
    public:
        ...
        static const int8_t *testDecodeFloat(float *value, const int8_t *buf, uint32_t size) {
        ...
        static void testMemoryCheck(const char *func_name, int8_t *start, int8_t *end,
                                    uint32_t len) {
            uint32_t used_len = end - start;

            if (used_len < len) {
                LOG_E("[GF_HAL][TestUtils]",
                      "[%s] alloced memery larger than used, used=%u, malloc=%u",
                      func_name, used_len, len);
            } else if (used_len > len) {
                LOG_E("[GF_HAL][TestUtils]", "[%s] memory out of bounds, used=%u, malloc=%u",
                      func_name,
                      used_len, len);
            }
        }
        static gf_error_t quantizationToByte(uint16_t *buf_src, uint16_t *buf_dst,
