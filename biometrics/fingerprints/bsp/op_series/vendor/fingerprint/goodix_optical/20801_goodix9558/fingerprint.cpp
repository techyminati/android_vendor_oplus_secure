...
#include "hal/inc/fp_hal_ext_service.h"
...
static int32_t setActiveGroup(struct fingerprint_device __unused *pDev,
                              uint32_t gid,
                              const char __unused *pStorePath) {
    assert(gpHalContext != nullptr);
...
{
    assert(gpHalContext != nullptr);
    ALOGV("authenticateastype %d",authtype);
...
{
    uint32_t total_enroll_times = 16;
...
static int fingerprint_pause_enroll(struct fingerprint_device __unused *dev)
{
    ...
    return gpHalContext->mFingerprintCore->update_status(0x09);
}

static int fingerprint_continue_enroll(struct fingerprint_device __unused *dev)
{
    ALOGV("fingerprint_pause_enroll");
    return gpHalContext->mFingerprintCore->update_status(0x08);
}
...
/*
    fingerprint_ext_device_t *ext_dev = (fingerprint_ext_device_t *) malloc(sizeof(fingerprint_ext_device_t));
    if (ext_dev != NULL) {
        ext_dev->dev = pDev;
        ext_dev->update_status = updateStatus;
        ext_dev->get_status = getStatus;
        ext_dev->get_eng_test = getEngEest;
        ext_dev->set_eng_notify = setEngNotify;
        ext_dev->start_eng_test = startEngTest;
        ext_dev->stop_eng_test = stopEngTest;
        ext_dev->send_command = sendFingerprintCmd;
        ext_dev->set_param = setParam;
        add_fingerprint_ext_service(ext_dev);
    }
   */
...
fingerprint_module_t HAL_MODULE_INFO_SYM = {
    .common = {
        ...
        .id = "fingerprint.20801_goodix9558_G3",
