...
namespace goodix {
    ...
    gf_error_t DelmarAlgo::authImage(gf_algo_auth_image_t *auth) {
        ...
        do {
            ...
            delmarAuthCmd->auth_time_flag = HalUtils::getCurrentTimeMicrosecond();
#ifdef SUPPORT_DSP_HAL
            err = DelmarAlgoUtils::authImageDsp(mContext, auth, mIsFirstAuth, mAuthSensorIds);
#else  // SUPPORT_DSP_HAL
            auth->header.target = GF_TARGET_ALGO;
            delmarAuthCmd->i_auth_sensor_ids = mAuthSensorIds;
            delmarAuthCmd->i_is_first_auth = mIsFirstAuth;
            delmarAuthCmd->i_temp_flag = DelmarAlgoUtils::detectTemperature();
