...
#define OPTICAL_SEPARATE_SOFTEATURE "ro.separate.soft"
...
namespace goodix {
    ...
    gf_error_t FingerprintCore::setNotify(gf_fingerprint_notify_t notify) {
        gf_error_t err = GF_SUCCESS;
        mNotify = notify;
        mContext->factory_notify = notify;
        return err;
    }
    ...
    uint64_t FingerprintCore::preEnroll() {
        ...
        LOG_E(LOG_TAG, "[%s] mPrenrollTime time is %d", __func__, mPrenrollTime);
        FUNC_EXIT(err);
    ...
    gf_error_t FingerprintCore::enroll(const void *hat, uint32_t gid,
        ...
        LOG_E(LOG_TAG, "[%s] now time is %d", __func__, now);
    ...
    gf_error_t FingerprintCore::sendFingerprintCmd(int32_t cmd_id, int8_t* in_buf, uint32_t size)
    {
        ...
        property_get(OPTICAL_SEPARATE_SOFTEATURE, separate_soft, "0");
        ...
        if(PRODUCT_TEST_CMD_PERFORMANCE_TESTING == cmd_id){
            switch (operationStep){
                ...
                case OPERATION_STEP_BASEDATA_MID_COLLECT:
                    setHbmMode(0);
                    if ((!strncmp(separate_soft, "19821", sizeof(19821)))) {
                        setBrightness(2860);
                    } else {
                        setBrightness(2040);
                    }
        ...
        else if (PRODUCT_TEST_CMD_GET_VERSION == cmd_id) {
            cmd_id = PRODUCT_TEST_CMD_SPI;
        }
        CustomizedDelmarProductTest *mProductTest = (CustomizedDelmarProductTest*)(*(mContext->mExtModuleList.end() - 1));
        ...
        if((PRODUCT_TEST_CMD_IMAGE_QUALITY == cmd_id)||(PRODUCT_TEST_CMD_AGE_STOP == cmd_id)){
            setHbmMode(0);
            LOG_I(LOG_TAG, "[sendFingerprintCmd] exit PRODUCT_TEST_CMD_IMAGE_QUALITY");
        }
    ...
    gf_error_t FingerprintCore::setHbmMode(uint32_t Mode)
    {
        ...
        LOG_E(LOG_TAG, "setHbmMode delay 300ms %d", Mode);
        usleep(300*1000);
        #endif
