...
namespace goodix {
    ...
    /*gf_error_t HalContextExt::createProductModule(HalContextExt* halContext) {
        gf_error_t err = GF_SUCCESS;
        FUNC_ENTER();
        LOG_D(LOG_TAG, "into createProductModule, %p", halContext->mProductTest);
        halContext->mProductTest = createProductTest(halContext);
        LOG_D(LOG_TAG, "into createProductModule, %p", halContext->mProductTest);
        if (NULL == halContext->mProductTest)
        {
