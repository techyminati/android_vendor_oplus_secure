...
char algo_package_version[] = "v03.01.02.12";
...
*/
    ...
    gf_error_t CustomizedFingerprintCore::prepareAuthRequest() {
        ...
        do {
            ...
            GF_ERROR_BREAK(err);
            sensor->getSpmtPassOrNot(&spmt_pass);
            if (0 == spmt_pass) {
...
*/
    ...
    gf_error_t CustomizedFingerprintCore::send_auth_dcsmsg(AuthenticateContext* context, bool notified) {
        ...
        if (mNotify != nullptr) {
            ...
            message.data.auth_dcsmsg.quality_score = dump_performance->image_quality[0][0];
            message.data.auth_dcsmsg.match_score = dump_performance->match_score[0][0];
            message.data.auth_dcsmsg.signal_value = 0;//invalid
            message.data.auth_dcsmsg.img_area = dump_performance->valid_area[0][0];
            ...
            message.data.auth_dcsmsg.chip_ic = 9608;
            message.data.auth_dcsmsg.module_type = 0;//invalid
            message.data.auth_dcsmsg.lense_type = 0;//invalid
            message.data.auth_dcsmsg.dsp_availalbe = 1;
            message.data.auth_dcsmsg.auth_total_time = (uint32_t)(mTotalKpiTime / 1000);
            message.data.auth_dcsmsg.ui_ready_time = 0;
            message.data.auth_dcsmsg.capture_time = dump_performance->get_raw_data_time / 1000;
            message.data.auth_dcsmsg.preprocess_time = dump_performance->preprocess_time[0] / 1000;
            message.data.auth_dcsmsg.get_feature_time = dump_performance->get_feature_time[0][0] / 1000;
            message.data.auth_dcsmsg.auth_time= dump_performance->authenticate_time[0][0] /1000;
            message.data.auth_dcsmsg.detect_fake_time = 0;
