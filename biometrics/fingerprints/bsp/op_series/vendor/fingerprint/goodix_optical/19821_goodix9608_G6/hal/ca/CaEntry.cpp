...
namespace goodix {
    ...
    gf_error_t CaEntry::startTap(bool isDSPEnabled) {
        ...
        if (err == GF_SUCCESS) {
            ...
            LOG_D(LOG_TAG, "19821 [%s] Load ta: %s succeeded.", __func__,
                  qcontext->customizedParams.goodixTaName);
            sendHmacKey(this);
            ...
            LOG_E(LOG_TAG, "19821 [%s] Load ta: %s failed, ret=%d, errno=%d.", __func__,
    ...
    gf_error_t CaEntry::sendCommand(void *cmd, uint32_t len) {
        ...
        do {
            ...
            LOG_E(LOG_TAG,"[%s] taLogDumpLevel is %d", __func__, taLogDumpLevel);
            if (taLogDumpLevel > 0) {
                taLogBufferLen = TA_LOG_DUMP_BUFFER_SIZE;
            }
            LOG_E(LOG_TAG,"[%s] taLogBufferLen = %d", __func__, taLogBufferLen);
			...
			LOG_E(LOG_TAG,"[%s] allocate ion memory success.", __func__);
