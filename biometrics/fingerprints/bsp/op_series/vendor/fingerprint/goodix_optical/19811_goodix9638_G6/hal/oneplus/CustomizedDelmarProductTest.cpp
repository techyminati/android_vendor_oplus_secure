...
namespace goodix {
    ...
    CustomizedDelmarProductTest::CustomizedDelmarProductTest(HalContext *context)
        ...
        LOG_D(LOG_TAG, "[%s] opticalType=%d, bgversion=%d", __func__, opticalType, sensorBgVersion);
        init(opticalType, sensorBgVersion);
    }
    ...
    void CustomizedDelmarProductTest::init(uint8_t opticalType, uint32_t bgVersion) {
        if (opticalType == DELMAR_OPTICAL_TYPE_3_0) {
            ...
            if (bgVersion == 0) {
                ...
                mNewTestConfig.minTSNRLow = 15.45;
...
*/
        switch (phase) {
            ...
            case OPERATION_STEP_CALCULATE_SIMPLIFIED_PERFORMANCE_INDICATORS: {
                ...
                LOG_D(LOG_TAG, "[%s]xy_info: standardCenterX= %f,standardCenterY= %f", __func__, mNewTestConfig.standardCenterX, mNewTestConfig.standardCenterY);
    ...
    void CustomizedDelmarProductTest:: notifySpiTestCmd(const int8_t *result, int32_t resultLen) {
        ...
        if (err == GF_SUCCESS) {
            eng_result.result = 1;
            snprintf(eng_result.info, sizeof(eng_result.info), "%s: SMT1_ID:%d,SMT2_ID:%d,FLASH ID:%d,SENSOR ID:%d,MCU ID:%d,SENSOR VERSION:%d,G6 LIGHT:3.0,",
                "PASS", sensor_info.o_vendor_id[0], sensor_info.o_vendor_id[1], flash_id, sensor_chip_id, mcu_chip_id, random_number);
        }
