...
namespace goodix {
    ...
    gf_error_t HalDsp::waitDspNotify() {
        ...
        do {
            ...
            } while (DsptryLockCount);

            if (err == GF_ERROR_DSP_NOT_AVAILABLE)
                break;
    ...
    int32_t HalDsp::convertChipType(gf_sensor_info_t info) {
        ...
        switch (info.chip_series) {
            ...
            case GF_DELMAR: {
                if ((info.chip_type & 0xFF) == 0x01) {
                    chipType = DSP_PROXY_CHIP_DELMAR_T;
                } else {
                    chipType = DSP_PROXY_CHIP_DELMAR_TA;
                }
                break;
            }
            default: {
