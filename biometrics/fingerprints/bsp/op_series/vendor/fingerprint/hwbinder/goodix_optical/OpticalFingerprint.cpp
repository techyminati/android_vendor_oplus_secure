...
fingerprint_device_t* OpticalFingerprint::openHal() {
    ...
    } else if ((FP_OPTICAL_GOODIX_19821_G5 == fp_config_info_init.fp_factory_type) || (FP_OPTICAL_GOODIX_19811_G5 == fp_config_info_init.fp_factory_type)) {
        if (0 != (err = hw_get_module("fingerprint.goodix9608_G5", &hw_mdl))) {
            ALOGE("Can't open fingerprint HW Module, error: %d", err);
            return nullptr;
        }
    } else if (FP_OPTICAL_GOODIX_19821_G6 == fp_config_info_init.fp_factory_type) {
        if (0 != (err = hw_get_module("fingerprint.goodix9638_G6", &hw_mdl))) {
            ALOGE("Can't open fingerprint HW Module, error: %d", err);
            return nullptr;
        }
    } else if (FP_OPTICAL_GOODIX_19811_G6 == fp_config_info_init.fp_factory_type) {
        if (0 != (err = hw_get_module("fingerprint.19811_goodix9638_G6", &hw_mdl))) {
            ALOGE("Can't open fingerprint HW Module, error: %d", err);
            return nullptr;
        }
    } else if (FP_OPTICAL_GOODIX_19805_G6_3 == fp_config_info_init.fp_factory_type) {
        if (0 != (err = hw_get_module("fingerprint.19805_goodix9638_G6_3", &hw_mdl))) {
            ALOGE("Can't open fingerprint HW Module, error: %d", err);
            return nullptr;
        }
    } else if (FP_OPTICAL_GOODIX_19805_G6_7 == fp_config_info_init.fp_factory_type) {
        if (0 != (err = hw_get_module("fingerprint.19805_goodix9678_G6_7", &hw_mdl))) {
            ALOGE("Can't open fingerprint HW Module, error: %d", err);
            return nullptr;
        }
    } else if (FP_OPTICAL_GOODIX_G6_3 == fp_config_info_init.fp_factory_type) {
        if (0 != (err = hw_get_module("fingerprint.19805_goodix9638_G6_3", &hw_mdl))) {// for 20838 fp second supply
            ALOGE("Can't open fingerprint HW Module, error: %d", err);
            return nullptr;
        }
    } else if (FP_OPTICAL_GOODIX_G6_7 == fp_config_info_init.fp_factory_type) { // 20838 fp first supply use 20828 fp hal
        // for 20838 fp first supply can also use "fingerprint.19805_goodix9678_G6_7"
        if (0 != (err = hw_get_module("fingerprint.20828_goodix9678", &hw_mdl))) {
            ALOGE("Can't open fingerprint HW Module, error: %d", err);
            return nullptr;
        }
    } else if (FP_OPTICAL_GOODIX_20801_G3 == fp_config_info_init.fp_factory_type) {
        if (0 != (err = hw_get_module("fingerprint.20801_goodix9558_G3", &hw_mdl))) {
            ALOGE("Can't open fingerprint HW Module, error: %d", err);
            return nullptr;
        }
    }
...
void OpticalFingerprint::notify(const fingerprint_msg_t *msg) {
    ...
    Return<int32_t> OpticalFingerprint::sendFingerprintCmd(int32_t cmdId,  const hidl_vec<int8_t>& in_buf ) {
        ...
        case fingerprint_cmd_Id_t::FINGERPRINT_CMD_ID_AUTHENTICATE_TYPE:
        {
            ...
            memcpy(&operationId, value, sizeof(uint64_t));
            value += sizeof(uint64_t);
            memcpy(&gid, value, sizeof(uint32_t));
