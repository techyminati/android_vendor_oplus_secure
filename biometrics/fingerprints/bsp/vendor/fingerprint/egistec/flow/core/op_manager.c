...
int opm_get_image(int opt_type, unsigned int *quality)
{
	...
	retval = transfer_data(OPERATION_TYPE, EX_CMD_GET_IMAGE, opt_type, 0, 0, NULL,
			       &len, (unsigned char *)&image_status);

	if (0 == retval) {
		*quality = image_status.status;
		g_img_qty_score = image_status.qty_score;
	}

	TIME_MEASURE_STOP(opm_get_image, "opm_get_image");
	return retval;
}
...
#define AUTH_TOKEN_DISABLE 0
	...
	if (0 == retval) {
		...
		*status = value[1];
		...
		verify_info->dcsmsg.quality_score = value[4];
		verify_info->dcsmsg.match_score = value[5];
		ex_log(LOG_INFO, "%s, match_id = %d, quality_score = %d, match_score = %d", __func__, value[0], value[5], value[6]);

		if (AUTH_TOKEN_ENABLE == auth_token_enable && out_data_len > sizeof(uint32_t) * 2) {
			token_len = value[3];
			egislog_d("opm_identify token_len = %d", token_len);
			if (token_len <= *out_token_size) {
