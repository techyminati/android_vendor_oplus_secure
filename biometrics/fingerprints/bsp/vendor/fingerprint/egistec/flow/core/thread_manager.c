...
static semaphore_handle_t g_tasks_sem[2];
...
void queue_init()
{
	...
	g_tasks_sem[0].sema = (void *)NULL;
	g_tasks_sem[1].sema = (void *)NULL;
...
#define WAIT_IDLE_TIME 500  //ms
	...
	ex_log(LOG_DEBUG, "*** queue_pop(%d, %d) enter", type, wait_type);
	...
	ex_log(LOG_DEBUG, "*** queue_pop() result = %d", result);
...
{
	ex_log(LOG_DEBUG, "*** queue_add(%d, %d) enter", task_type, task);
	switch (task_type) {
		case TASK_Q:
			g_task = task;
	...
	ex_log(LOG_DEBUG, "*** queue_add(%d, %d) end", task_type, task);
	return 0;
}
/*
...
void thread_manager_run_task(do_operation_callback lpAddress, task_t task)
{
	...
	ex_log(LOG_DEBUG, "+++ queue_pop(AVAILABLE_Q, WAIT_IMMEDIATELY) ");
	result = queue_pop(AVAILABLE_Q, WAIT_IMMEDIATELY);
	switch (result) {
		case POP_TIMEOUT:
			//start cancel
...
static void clear_task(__unused task_t task)
{
...
{
	int result;
	ex_log(LOG_DEBUG, "+++ queue_pop(TASK_Q, %s) ", idle_excuted ? "WAIT_FOREVER" : "WAIT_IDLE");
...
static void *thread_manager(void)
{
	...
	while (1) {
		ex_log(LOG_DEBUG, "NEW LOOP");
		task = get_task(idle_excuted);
		ex_log(LOG_DEBUG, "going to do task = %d", task);
		...
		switch (task) {
			...
			case TASK_IDLE:
				...
				idle_excuted = TRUE;
				ex_log(LOG_DEBUG, "=== TASK_IDLE end");
				break;

			case TASK_PROCESS:
				ex_log(LOG_VERBOSE, "=== TASK_PROCESS start");
				if (g_pflpAddress == NULL) {
					goto exit;
				}
				result = g_pflpAddress();
				ex_log(LOG_DEBUG, "=== TASK_PROCESS end, result=%d", result);
				break;

			case TASK_CANCEL:
				/* defines for logger, works as the same as TASK_EMPTY*/
				ex_log(LOG_DEBUG, "=== TASK_CANCEL");
				break;
			case TASK_EXIT:
				ex_log(LOG_DEBUG, "=== TASK_EXIT");
				goto exit;
			default:
		...
		ex_log(LOG_DEBUG, "task excuted");
		clear_task(task);
		ex_log(LOG_DEBUG, "task excuted , clear");
	}
exit:
	ex_log(LOG_ERROR, "thread_manager end, retval = %d", result);
...
int thread_try_lock_operation(void)
{
	...
	ex_log(LOG_DEBUG, "thread_try_lock_operation enter");
	...
	ex_log(LOG_DEBUG, "thread_try_lock_operation end, retval = %d", retval);
	return retval;
}
...
{
	ex_log(LOG_DEBUG, "thread_unlock_operation enter");
	queue_add(TASK_Q, TASK_EMPTY);
	ex_log(LOG_DEBUG, "thread_unlock_operation end");
}
...
int trylock_operation(rbs_operation_t operation)
{
	...
	ex_log(LOG_DEBUG, "trylock_operation = %d, retval = %d", operation, retval);
	return retval;
}
...
int unlock_operation(rbs_operation_t operation)
{
	...
	ex_log(LOG_DEBUG, "unlock_operation = %d ,ret %d", operation, retval);
	return retval;
}
