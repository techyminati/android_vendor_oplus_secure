...
static int _map_ini_handler(__unused void* user, const char* section, const char* name,
	const char* value){
...
#ifdef QSEE
	egist_snprintf(map_key, key_len, "%s_%s", section, name);
	egist_snprintf(map_value, MAX_VALUE, "%s", value);
#elif __TRUSTONIC__
	strncpy(map_key, section, key_len - 1);
	strcat(map_key, "_");
	strcat(map_key, name);
	strncpy(map_value, value, MAX_VALUE - 1);
#else
	snprintf(map_key, key_len, "%s_%s", section, name);
	snprintf(map_value, MAX_VALUE, "%s", value);
#endif

	hashmap_put(g_map, map_key, map_value);
	egislog_d("put %s ,value %s ", map_key, map_value);
...
#ifdef QSEE
	egist_snprintf(key, key_len, "%s_%s", setction, in_name);
#elif __TRUSTONIC__
	strncpy(key, setction, key_len - 1);
	strcat(key, "_");
	strcat(key, in_name);
#else
	snprintf(key, key_len, "%s_%s", setction, in_name);
#endif
...
static void _init_helper_value(struct _iniHelperValue* iniHelperValue, const char* section,
			       const char* name, int data_type, void* out_value, int out_string_max)
{
...
void check_ini_version(){
	if (g_ini_string != NULL){
		int version_flag = ini_parser_get_int(INI_SECTION_SENSOR, KEY_INI_VERSION_FLAG, 0);

		if (version_flag != INI_VERSION_FLAG){
			egislog_e("rbs_config.ini version not matched!");
		}
		else {
...
#endif
	egislog_d("%s [%s] %s => %d,default %d \n", __func__, section, name, ret_value, default_value);

	return ret_value;
}
int ini_parser_get_string(const char* section,const char* name, char* out_value, int size,const char* default_value)
{
    if (g_ini_string == NULL) {
    	return EGIS_CANCEL;
    }
	...
	value = int_get_value(section, name);
	if (value == NULL){
		strncpy((char*)out_value, default_value, size);
	}else
		strncpy((char*)out_value, value, size);
#else
	struct _iniHelperValue helperValue;
	...
	if (helperValue.found) {
		...
		strncpy(out_value, default_value, size);
		return EGIS_OK;
	}
#endif
	egislog_d("%s [%s] %s => %s,default %s \n", __func__, section, name, out_value, default_value);
