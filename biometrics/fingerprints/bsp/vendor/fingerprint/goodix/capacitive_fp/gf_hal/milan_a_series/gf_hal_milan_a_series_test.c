...
#define LOG_TAG "[GF_HAL][gf_hal_milan_a_series_test]"
...
static void irq_hbd_test(gf_irq_t *cmd, gf_error_t err_code);

static void irq_hbd_test(gf_irq_t *cmd, gf_error_t err_code)
{
    if (OPERATION_TEST_BIO_CALIBRATION == cmd->operation)
    {
        hal_notify_test_bio_hbd_calibration(err_code, &cmd->test_hdb_feature,
                                            CMD_TEST_BIO_CALIBRATION);
    }
    else if (OPERATION_TEST_HBD_CALIBRATION == cmd->operation)
    {
        hal_notify_test_bio_hbd_calibration(err_code, &cmd->test_hdb_feature,
...
static void irq_image_test(gf_irq_t *cmd, gf_error_t *err_code)
{
    ...
    switch (cmd->operation)
    {
        ...
        case OPERATION_TEST_SPI_TRANSFER:
        {
            ...
            if ((gf_hal_current_time_microsecond() - g_irq_time_milan_a) > 800000)
            {
