 ...
 * All Rights Reserved.
 * Description: gf_hal test utils header file
 * History:
 * Version: 1.0
 */

#ifndef _GF_HAL_TEST_UTILS_H_
#define _GF_HAL_TEST_UTILS_H_
...
uint8_t *hal_test_encode_array(uint8_t *buf, uint32_t token, uint8_t *array,
                               uint32_t size);
uint8_t *hal_test_encode_array_minus(uint8_t *buf, uint32_t token, uint8_t *array,
                               uint32_t size);
