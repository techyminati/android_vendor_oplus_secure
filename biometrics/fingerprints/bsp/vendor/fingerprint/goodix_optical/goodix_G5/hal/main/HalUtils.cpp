...
// battery
#define TEMPRATURE_DEVICE_PATH "sys/class/power_supply/battery/temp"
...
namespace goodix {
    ...
    int32_t HalUtils::detectTemperature() {
        FILE* fp = NULL;
        char buffer[80] = { 0 };
        int32_t temperature = 0;
        do {
            LOG_D(LOG_TAG, "[%s] Open %s.", __func__, TEMPRATURE_DEVICE_PATH);
            ...
            LOG_I(LOG_TAG, "detectT  = %d", temperature);
        } while (0);
        if (fp != NULL) {
            fclose(fp);
            fp = NULL;
