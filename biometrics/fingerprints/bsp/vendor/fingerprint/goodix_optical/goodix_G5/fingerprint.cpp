...
{
    assert(gpHalContext != nullptr);
    ALOGV("authenticateastype");
    return gpHalContext->mFingerprintCore->authenticate(operationId, gid, authtype);
}
/**
 * Function: authenticate
 * Description: authenticate for vts debug
 * Input: pDev, operationId, gid
 * Output: none
 * Return:
 * 0       if succeed
