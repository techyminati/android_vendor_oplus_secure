...
namespace goodix {
...
#if 0  // customied for support muti platform
    const int32_t gHexagonVersion = 65;  // hexagon v65
#else  // customied for support muti platform
    const int32_t gHexagonVersion = 66;  // hexagon v66
#endif  // customied for support muti platform
    ...
    int (*gf_proxy_set_high_dsp_freq)(int32_t, int32_t, int32_t, int32_t) = NULL;
    ...
    HalDsp::HalDsp(CaEntry *caEntry) : mDspStatus(DSP_NOT_AVAILABLE), mDspSpeedStatus(DSP_NORMAL_SPEED), mDspResult(0), mChipType(0) {
        mCaEntry = caEntry;
    ...
    gf_error_t HalDsp::proxyHandleInit(void) {
        ...
        do {
            ...
            gf_proxy_set_high_dsp_freq = (int (*)(int32_t, int32_t, int32_t, int32_t))dlsym(g_proxy_handle, "gf_proxy_set_high_dsp_freq");
    ...
    gf_error_t HalDsp::init(gf_sensor_info_t info) {
        ...
        do {
            ...
            mChipType = convertChipType(info);
    ...
    gf_error_t HalDsp::sendCmdToDsp(gf_dsp_cmd_t cmd) {
        ...
        do {
            ...
            switch (cmd) {
                ...
                case DSP_CMD_START_LISTEN_TO_TA: {
                    clearDspSem();

                    proxy_cmd.type = DSP_PROXY_SEND_CMD;
                    proxy_cmd.cmd_type = SEC_FP_DSP_START_GET_TA_CMD;
                    err = dspProxySendCmd(&proxy_cmd);
                    break;
                }
    ...
    gf_error_t HalDsp::setDspHighFreq() {
        ...
        if (gf_proxy_set_high_dsp_freq != NULL) {
            ret = gf_proxy_set_high_dsp_freq(0, 1000, 0, gHexagonVersion);
            if (0 != ret) {
                LOG_E(LOG_TAG, "[%s] hal set high dsp freq fail", __func__);
                err = GF_ERROR_BASE;
            } else {
                LOG_D(LOG_TAG, "[%s] hal set high dsp freq success", __func__);
    ...
    gf_error_t HalDsp::setDspNormalFreq() {
        ...
        if (gf_proxy_set_normal_dsp_freq != NULL) {
            ...
            if (0 != ret) {
                ...
                LOG_D(LOG_TAG, "[%s] hal set normal dsp freq success", __func__);
            ...
            switch (mProxyCmd.type) {
                ...
                case DSP_PROXY_SET_HIGH_FREQ: {
                    ret = gf_proxy_set_high_dsp_freq(mProxyCmd.high_freq.power_level,
                                                     ...
                                                     mProxyCmd.high_freq.dcvs_enable, gHexagonVersion);
                    if (ret) {
                        reinit_flag = 1;
                        mProxyCmd.status = SEND_CMD_ERROR;
                        break;
    ...
    int32_t HalDsp::onDspCmdResult(int cmd, int result) {
        ...
        do {
            switch (cmd) {
                ...
                case SEC_FP_DSP_START_GET_TA_CMD: {
                    postFlag = 1;
                    break;
                }
                ...
                case SEC_FP_HVX_GET_VERSION: {
                    postFlag = 1;
                    LOG_D(LOG_TAG, "[%s] dsp version:%d", __func__, result);
                    break;
                }
    ...
    int32_t HalDsp::convertChipType(gf_sensor_info_t info) {
        ...
        switch (info.chip_series) {
            case GF_SHENZHEN: {
                chipType = DSP_PROXY_CHIP_SHENZHEN;
                break;
            }
            case GF_DELMAR: {
                if ((info.chip_type & 0xFFF) == 0x001) {
                    chipType = DSP_PROXY_CHIP_DELMAR_G5;
                } else if ((info.chip_type & 0xFFF) == 0x003) {
                    chipType = DSP_PROXY_CHIP_DELMAR_G6;
                } else if ((info.chip_type & 0xFFF) == 0x103) {
                    chipType = DSP_PROXY_CHIP_DELMAR_G7;
                } else {
                    chipType = DSP_PROXY_CHIP_DELMAR_G7;
                    LOG_E(LOG_TAG, "[%s] not support chip type :0x%x", __func__, info.chip_type);
                }
                break;
            }
            default: {
