...
namespace goodix {
    ...
    public:
        ...
        virtual gf_error_t getCaliLoadError();
        ...
        void getCoordinateInfo(gf_delmar_coordinate_info_t *coordinate_info);
        ...
        inline const uint8_t *getSensorId() {
            return (const uint8_t *)mSensorId[0];
        }
        ...
        inline uint32_t getCChipType() {
            return mCChipType;
        }

        inline void setValidEnrollPressCount(uint32_t count) {
            mValidEnrollPressCount = count;
        }

        inline void setLBMode(uint32_t mode) {
            mLowBrightnessMode = mode;
        }

        inline uint32_t getLBMode() {
            return mLowBrightnessMode;
        }

        inline void getQrCodeData(const uint8_t **qrCode, uint32_t *len) {
            *qrCode = mQrCodeData;
            *len = mQrCodeDataLen;
        }

        inline void setInHBMode(bool mode) {
            mIsInHBMode = mode;
        }
    ...
    protected:
        ...
        virtual uint8_t getDeviceStatus(void);
        virtual int32_t getLightSensorVal(void);
    ...
    private:
        ...
        uint8_t mSensorId[MAX_SENSOR_NUM][DELMAR_SENSOR_ID_MAX_BUFFER_LEN];
        uint32_t mSensorIdLen;
        uint32_t mSensorBgVersion;
        uint32_t mCChipType;
        uint32_t mValidEnrollPressCount;
        uint32_t mLowBrightnessMode;
        uint8_t mQrCodeData[MAX_QR_CODE_INFO_LEN];
        uint32_t mQrCodeDataLen;
        bool mIsInHBMode;
    };
}  // namespace goodix
