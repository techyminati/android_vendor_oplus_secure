 ...
 ** --------------------------- Revision History: --------------------------------
 **  <author>           <data>            <desc>
 **  Ran.Chen         2019/10/17        modify for temperature value
 **  Bangxiong.Wu     2019/10/23        add function getrealtime for enroll timer
 ************************************************************************************/
#include <errno.h>
#include <sys/time.h>
#include <time.h>
...
#define LOG_TAG "[GF_HAL][HalUtils]"
...
namespace goodix {
    ...
    long long HalUtils::getTimeUs() {
        struct timeval now;
        memset(&now, 0, sizeof(timeval));
        gettimeofday(&now, 0);
        /* fixed coverity: now.tv_sec long -> long long */
        return ((long long)now.tv_sec) * 1000000L + now.tv_usec;
    }

    #define ANDROID_TARGET_MODE_RELEASE 0
    #define ANDROID_TARGET_MODE_USERDEBUG 1
    AndroidVersionType HalUtils::getAndroidVersionType() {
        const char *name = "ro.oplus.image.my_engineering.type";
        AndroidVersionType type = FP_RELEASE;
        char versionType[PROPERTY_VALUE_MAX];
        (void)property_get(name, versionType, "release");

    #if (ANDROID_VERSION_MODE == ANDROID_TARGET_MODE_USERDEBUG)
        type = FP_USERDEBG;
    #else
        if (strcmp(versionType, "preversion") == 0) {
            type = FP_PREVERSION;
        } else if (strcmp(versionType, "factory") == 0) {
            type = FP_FACTORY;
        } else {
            type = FP_RELEASE;
        }
    #endif
        LOG_I(LOG_TAG, "[%s] android versoin:%d", __func__, type);
        return type;
    }

    long long HalUtils::getFileSize(const char* path) {
        if (path == 0) {
        ...
        struct stat st;
        if (stat(path, &st) != 0) {
            LOG_E(LOG_TAG, "stat err:%s", strerror(errno));
            return 0;
        }
...
int HalUtils::getDirFileInfo(const char *path, int* totalCount,
    unsigned int* totalSize, char* earliestFile, char* fileFilter) {
    int err = 0;
    long mtime = 0;

    LOG_D(LOG_TAG, "path:%s", path);
    if (!path || !totalCount || !totalSize || !earliestFile) {
        LOG_E(LOG_TAG, "error input, path:%p totalCount:%p totalSize:%p earliestFile:%p",
        path, totalCount, totalSize, earliestFile);
        err = -1;
        goto fp_out;
    }
    ...
    *totalSize = 0;
    ...
    err = getDirFileInfoInner(path, totalCount, totalSize, earliestFile, fileFilter, &mtime);

fp_out:
    return err;
}

int HalUtils::getDirFileInfoInner(const char *path, int* totalCount,
    ...
    dir = opendir(path);
    if (dir == NULL) {
        LOG_E(LOG_TAG, "opendir err:%s", strerror(errno));
        err = -1;
        goto fp_out;
    }
    ...
    while ((ent = readdir(dir)) != NULL) {
        ...
        if (ent->d_type & DT_DIR) {
            getDirFileInfoInner(fullPath, totalCount, totalSize, earliestFile, fileFilter, mtime);
        } else {
            struct stat st;
            /* fixed: need fullPath */
            ...
            *totalCount += 1;
            *totalSize += (unsigned int)st.st_size;

            if (fileFilter == NULL) {
                if (*mtime == 0) {
                    *mtime = st.st_mtime;
                    snprintf(earliestFile, 256, "%s/%s", path, ent->d_name);
                } else {
                ...
                if (strstr(path, fileFilter) != NULL) {
                    if (*mtime == 0) {
                        *mtime = st.st_mtime;
                        snprintf(earliestFile, 256, "%s/%s", path, ent->d_name);
                    } else {
            ...
            #if 0
            LOG_E(LOG_TAG, "total:%d find file:%s size:%u mtime:%ld",
                *totalCount, ent->d_name, st.st_size, st.st_mtime);
            #endif
        }
    }
    closedir(dir);
...
int HalUtils::removePath(const char* path) {
    if (path == 0) {
        LOG_E(LOG_TAG, "path is NULL");
        return -1;
    }
