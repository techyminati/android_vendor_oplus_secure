...
namespace goodix {
    ...
    gf_error_t SensorDetector::detectSensor(gf_sensor_info_t *info) {
        ...
        do {
            ...
            /* expand deal: especial for FTM mode */
            property_get("ro.boot.mode", (char*)cmd.reserve, "\0");
            LOG_I(LOG_TAG, "[%s] ro.boot.mode:%s", __func__, cmd.reserve);
            /* expand deal: especail for FTM mode */
