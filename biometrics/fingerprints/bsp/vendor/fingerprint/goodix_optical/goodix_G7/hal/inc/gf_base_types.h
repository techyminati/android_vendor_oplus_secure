...
#endif  //  MAX_FINGERS_PER_USER

#define QSEE_HMAC_KEY_MAX_LEN (256)

#define MAX_RESERVE_SIZE (128)
...
#define GF_DATA_SYNC_FILE       (1)
#define GF_DATA_SYNC_MT_CALI    (2)
...
typedef struct {
    ...
    uint32_t cmd_id;
    gf_error_t result;
    uint8_t persist_data_change;    //  1:sync file from tee;2: sync cali data from tee
...
typedef enum {
    GF_CMD_DATA_SYNC_INIT_INJECT,
    GF_CMD_DATA_SYNC_FROM_TEE,
    GF_CMD_DATA_SYNC_MTCALI_TO_TEE,
    GF_CMD_DATA_SYNC_CALI_FROM_TEE,
    GF_CMD_DATA_SYNC_MAX
} gf_data_sync_cmd_id_t;

typedef struct gf_data_sync_info {
    gf_cmd_header_t header;
    uint8_t first_trans;  // first data trans
    uint8_t buffer[CA_TO_TA_MAX_BUFFER];
    uint32_t actual_len;
    uint32_t total_len;
    uint32_t remaining_len;  // indicate the remaining data will  be transfered from ta to ca
    uint32_t remaining_file_count;
    uint8_t reserve[MAX_RESERVE_SIZE];
} gf_data_sync_info_t;
