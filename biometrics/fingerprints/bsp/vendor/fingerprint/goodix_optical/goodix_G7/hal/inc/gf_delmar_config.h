...
typedef struct gf_delmar_config {
    ...
    uint32_t fake_opt_enable_mask;  // control fake detect module
    int32_t fake_safe_level;
    int32_t safe_class;
    ...
    uint8_t support_retry_highlight_base;  // 0: not support 1: support 2: support but enroll not return GF_ERROR_HIGH_LIGHT
    uint8_t support_otp_update_on_calibration;  // 1:support update, 0: not support update
    uint16_t moire_filter_config;
    float exp_time;
    float lb_exp_time;  //  low brightness exposure time
    float lb_lexp_time;  //  low brightness long exposure time
    float lexp_time;
    float hb_exp_time;
    float hb_hl_exp_time;  // hear trate high light exposure time
    ...
    uint8_t support_slide_check;  // only G6X(TSMC) support slide check
    ...
    uint8_t support_skip_some_auth_process;
    uint8_t support_retry1_hl_double_frame_average;  // only g6x support retry1 highlight 2 frames
    uint8_t support_low_brightness_mode;
    uint8_t support_local_hbm_mode;
    uint16_t hb_sample_count;  // heart rate sample count. max is 20s min is 4s
    uint8_t check_regist_replace_mode;
    uint8_t support_expand_enable;
    int32_t enroll_anomal_check_threshold_1;
    int32_t enroll_anomal_check_threshold_2;
    int32_t support_flesh_3D_fool_proof;
    uint8_t enable_big_data;

    uint8_t disable_dual_finger_study;
    uint8_t support_retry_expo_in_advance;
    uint8_t support_fu_product_test;
    uint8_t support_retry1_auth_only_prior_fingers;  // When five fingers are enrolled, only the prior fingers are recognized on retry1
    uint8_t support_highlight_by_light_sensor;
    uint8_t support_record_pt_data;
    uint8_t support_dsp_fake;
    uint8_t support_scan_expo_time;
    uint8_t load_persist_no_sensor_id_checked;
    uint8_t support_reset_logo_base;
    int32_t badpoint_filter_radius_1;
    uint8_t support_calc_expo_by_transmissivity;
    uint8_t support_adapt_expo_for_pwm;
    float normal_brightness_nit;
    float low_brightness_nit;
    int32_t antipeep_ratio_threshold;
    uint8_t support_decrease_expo_for_product_test;
    uint8_t support_beta_tryout;
    uint8_t support_use_dataprocess_sub_dark_for_product_test;
    uint8_t reserve[MAX_RESERVE_SIZE * 4 - 22];
} gf_delmar_config_t;
...
typedef enum {
    ...
    DELMAR_SENSOR_TYPE_SINGLE_CS_SE3 = 0x00000103,
