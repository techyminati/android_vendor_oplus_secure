...
namespace goodix {
    ...
    gf_error_t DelmarFingerprintCore::onAfterEnrollNotify(EnrollContext *context) {
        ...
        do {
            ...
            if (delmar_cmd->io_enr_stage != 0 && delmar_cmd->common.o_samples_remaining > 0) {
    ...
    gf_error_t DelmarFingerprintCore::onBeforeAuthCapture(AuthenticateContext *context) {
        ...
        do {
            if (config->support_independent_fast_auth > 0 && context->retry == 1) {
                if (cmd->o_force_retry > 0) {
                    context->retry++;  // need capture
                } else {
                    context->result = err;
                    break;  // no need capture
                }
            }
            err = clearSensorIds();
            if (err != GF_SUCCESS) {
    ...
    gf_error_t DelmarFingerprintCore::onAfterAuthCapture(AuthenticateContext *context) {
        ...
        do {
            if (context->result != GF_SUCCESS) {
                // if capture image failed, do not read image
    ...
    void DelmarFingerprintCore::onError(gf_error_t err) {
        if (err == GF_ERROR_UI_READY_TIMEOUT) {
            ...
            notifyErrorInfo(GF_FINGERPRINT_ERROR_TIMEOUT);
        } else {
    ...
    bool DelmarFingerprintCore::needRetryByResult(HalContext *context, gf_error_t result,
        ...
        uint8_t authSupportLongExpo = delmarConfig->support_long_exposure & 0x01;
        bool isOptical7_X = DelmarHalUtils::isOptical7X(((DelmarSensor*)context->mSensor)->getOpticalType());
        bool needRetry = (result == GF_ERROR_NOT_MATCH
            ...
            || (result == GF_ERROR_NOT_LIVE_FINGER && !isOptical7_X && authSupportLongExpo == 0
    ...
    bool DelmarFingerprintCore::needRetry(AuthenticateContext *context) {
        ...
        do {
            ...
            if (getSensor()->getLBMode() && delmarConfig->lb_lexp_time < 0.5) {
                LOG_E(LOG_TAG, "[%s] LB mode, don't set lb lexp time no need retry.", __func__);
                needRetry = false;
                break;
            }
        ...
        if (delmarConfig->support_independent_fast_auth > 0 && cmd->common.i_retry_count == 0
                && needRetry && cmd->o_force_retry == 0) {
            gf_delmar_sensor_ids_t ids_cmd = { {0} };
            gf_error_t tmp_err = GF_SUCCESS;
            ids_cmd.study_sensor_ids = 0;
            ids_cmd.valid_sensor_ids = 0;
            tmp_err = getAlgo()->updateSensorIds(&ids_cmd);
            LOG_D(LOG_TAG, "[%s] ignore dump of retry0 if retry1 is required in independent fast auth mode, err: %d.",
                    __func__, tmp_err);
        }
