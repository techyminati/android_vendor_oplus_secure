...
#if TARGET_ION_ABI_VERSION >= 3 && defined(USE_DMA_MEMORY)
...
#define LOG_TAG "[GF_HAL][IonMemory]"

#if TARGET_ION_ABI_VERSION >= 3 && defined(USE_DMA_MEMORY)
#define CLOSE_FD close
#define DEFAULT_ALLOCATE_SIZE    (2048 * 1024)
#elif TARGET_ION_ABI_VERSION >= 2
...
#define ION_HEAPS(bit) ION_BIT(bit)
#endif
// the size of current default buffer
static int32_t g_cur_buf_size = 0;

#if TARGET_ION_ABI_VERSION >= 3 && defined(USE_DMA_MEMORY)
static BufferAllocator* g_bufferAllocator = NULL;
#endif  // end
...
namespace goodix {
    ...
    gf_error_t IonMemory::init() {
        ...
        do {
...
#ifndef CUSTOMIZED_ION_QSECOM_HEAP_ID
            unsigned int heap_id = ION_QSECOM_HEAP_ID;
#else  // CUSTOMIZED_ION_QSECOM_HEAP_ID
            unsigned int heap_id = CUSTOMIZED_ION_QSECOM_HEAP_ID;
#endif  // CUSTOMIZED_ION_QSECOM_HEAP_ID
            LOG_E(LOG_TAG, "[%s] heap_id=%d", __func__, heap_id);
            int32_t ret = -1;
            g_bufferAllocator = CreateDmabufHeapBufferAllocator();
            if (g_bufferAllocator == NULL) {
                err = GF_ERROR_GENERIC;
                LOG_E(LOG_TAG, "[%s] CreateDmabufHeapBufferAllocator() failed", __func__);
                break;
            }
            ret = MapDmabufHeapNameToIonHeap(g_bufferAllocator,
                                                                                                            "qcom,qseecom",
                                                                                                            "qsecom",
                                                                                                            0,
                                                                                                            ION_HEAP(heap_id),
            ...
            if (ret < 0) {
                err = GF_ERROR_GENERIC;
                LOG_E(LOG_TAG, "[%s] MapDmabufHeapNameToIonHeap() failed", __func__);
                break;
            }
            mIonDeviceFd = DmabufHeapAlloc(g_bufferAllocator, "qcom,qseecom", DEFAULT_ALLOCATE_SIZE, 0, 0);
    ...
    gf_error_t IonMemory::doAllocate(int32_t size, BUFFER_INFO *info) {
        ...
        pExtInfo->base.bufferFd = mIonDeviceFd;  // map fd
        ...
        ret = ioctl(mIonDeviceFd, DMA_BUF_IOCTL_SYNC, &buf_sync);
...
map_fail:
    ...
    gf_error_t IonMemory::doFree(BUFFER_INFO *info) {
        ...
        if (pExtInfo->base.bufferFd > 0) {  // map fd
            CLOSE_FD(pExtInfo->base.bufferFd);
        }

        if (g_bufferAllocator != NULL) {
            FreeDmabufHeapBufferAllocator(g_bufferAllocator);
            g_bufferAllocator = NULL;
        }
    ...
    gf_error_t IonMemory::carveoutIonMemalloc(uint32_t size) {
        ...
        do {
            ...
            map_fd = DmabufHeapAlloc(g_bufferAllocator, "qcom,qseecom", len, 0, 0);

            mCarveoutIonHandleFd = 0;

            if (ret || !map_fd) {
                LOG_E(LOG_TAG, "[%s] Error::DmabufHeapAlloc for size %u len %u failed ret = %d\n", __func__, size, len, ret);
                err = GF_ERROR_GENERIC;
                break;
            }
    ...
    gf_error_t IonMemory::doAllocate(int32_t size, BUFFER_INFO *info) {
        ...
        ret = ion_alloc_fd(mIonDeviceFd, len, align, ION_HEAPS(heap_id), flags, &map_fd);
...
alloc_fail:
    ...
    gf_error_t IonMemory::carveoutIonMemalloc(uint32_t size) {
        ...
        do {
            ...
            mCarveoutIonHandleFd = 0;
            ret = ion_alloc_fd(ion_fd, len, align, ION_HEAPS(15), flags, &map_fd);
