...
namespace goodix {
    ...
    static int32_t doWait(sem_t* sem, uint32_t ms) {
        ...
        while (semvalue > 0) {
            LOG_D(LOG_TAG, "[%s]Consume sem value 0 (%d).", __func__, semvalue);
            /* wait for sem.semvalue > 1, finger down/up event, then sem.semvalue = 0
             * [Notice] time:UTC Can't change the system time
             */
            sem_timedwait(sem, &t);
            sem_getvalue(sem, &semvalue);
        }

        /* wait for sem.semvalue > 1, uiready event, then sem.semvalue = 0 */
    ...
    bool Sensor::waitSensorUIReady() {
        ...
        do {
            ...
            int ret = doWait(&mUiReadySem, WAIT_UIREADY_TIMEOUT_MS);
            if (ret != 0 && errno == ETIMEDOUT) {
                LOG_E(LOG_TAG, "Wait ui ready timeout.");
                mIsUIReadyTimeout = true;
            }
        } while (0);
        VOID_FUNC_EXIT();
