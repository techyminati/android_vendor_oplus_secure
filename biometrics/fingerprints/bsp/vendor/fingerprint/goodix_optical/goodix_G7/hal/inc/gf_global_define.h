...
#define GF_TRACE_MEM_INFO()
    ...
    #if defined(_HALCONTEXT_CPP_) || defined(_EXTMODULE_CREATOR_CPP_)
        #include "Heartbeat.h"
    #endif  // _HALCONTEXT_CPP_ || _EXTMODULE_CREATOR_CPP_
    #undef HAL_CONTEXT_HEARTBEATRATE_CREATE
    #define HAL_CONTEXT_HEARTBEATRATE_CREATE \
        module = createHeartbeatRate(this); \
        if (module != nullptr) { \
            LOG_D(LOG_TAG, "[%s] add heartbeat rate into list", __func__); \
            mExtModuleList.push_front(module); \
            module->setNotify((ext_module_callback)callback); \
        }
    #undef GF_MODULES_DECLARE_HEARTBEAT_RATE
    #define GF_MODULES_DECLARE_HEARTBEAT_RATE extern gf_module_t g_heartbeat_rate_module;

    #undef GF_MODULES_REF_HEARTBEAT_RATE
    #define GF_MODULES_REF_HEARTBEAT_RATE &g_heartbeat_rate_module,
