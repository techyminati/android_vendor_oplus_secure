...
#define DELMAR_FAKE_LOG_ARRAY_SIZE                 (40)
...
#define DELMAR_AUTOCALI_PARAM_BUFFER_LEN           (888945)  // G5:888945, G6:635535
...
#define DELMAR_OAKLAND_CF_POINT_OUTER_CIRCLE      (792)  // (100*2 + 98*2)*sizeof(uint16_t)
...
#define DELMAR_DEFAULT_FAKE_CALIBRATION_DATA_LEN        (79413)
#define DELMAR_SA_FAKE_CALIBRATION_DATA_LEN             (74199)
#define DELMAR_OAK_FAKE_CALIBRATION_DATA_LEN            (55213)

#define DELMAR_SA_SLIDE_CHECK_DATA_LEN             (31200)  // 120 * 130 * 2
#define DELMAR_SLIDE_CHECK_DATA_LEN             DELMAR_SA_SLIDE_CHECK_DATA_LEN

#define G6G6X_MOIRE_COORDINATE_LEN        (6)
#define MAX_MOIRE_COORDINATE_LEN          (20)
#define SCAN_EXPO_TIME_MAX_FRAME_NUM      (20)
...
#define GF_DSP_FAKE_OVERLAPSIZE_G6   (90 * 90)
#define GF_DSP_FAKE_OVERLAPSIZE_G7   (65 * 65)
#define GF_DSP_FAKE_OVERLAPSIZE_G6X   (88 * 84)
...
#endif  // ifndef ABS
#define DELMAR_MAX_GUARD_BUF_LEN                   (400)

#define HEART_BEAT_RATE_SAMPLING_RATE       (10)
#define HEART_BEAT_RATE_MIN_CALC_TIME       (4)
#define HEART_BEAT_RATE_MAX_CALC_TIME       (20)
#define HEART_BEAT_RATE_MIN_SAMPLE_COUNT    (HEART_BEAT_RATE_SAMPLING_RATE * HEART_BEAT_RATE_MIN_CALC_TIME)
#define HEART_BEAT_RATE_MAX_SAMPLE_COUNT    (HEART_BEAT_RATE_SAMPLING_RATE * HEART_BEAT_RATE_MAX_CALC_TIME)
#define HL_HB_EXP_TIME                                             (50)

#define QRCODE_DATA_LEN                      (11)
#define MAX_QR_CODE_INFO_LEN                 (32)

#define DSP_STATUS_ERROR                    (1)

#define HBM_MODE_GLOBAL     1
#define HBM_MODE_LOCAL      0
#define HBM_MODE_UNKNOWN    255

#define DSP_AUTH_ENABLE_MASK_BIT                   0x01
#define DSP_ENROLL_ENABLE_MASK_BIT                 0x02
...
typedef enum {
    ...
    GF_CMD_DELMAR_GET_CALI_LOAD_ERROR = 103,
...
enum {
    ...
    DELMAR_CF_MASK_TYPE_OUTER_CIRCLE_INTERVAL_24_2,  // outer circle interval 24_2
};

enum {
    DELMAR_C_CHIP_TYPE_UNKOWN = 0x00,
    DELMAR_C_CHIP_TYPE_GW183,
    DELMAR_C_CHIP_TYPE_GW183D_ECO1,
    DELMAR_C_CHIP_TYPE_GW183D_ECO2,
};
...
typedef struct {
    ...
    uint8_t brightness_level;   //  unused member
    uint8_t lb_mode;
    uint32_t valid_enroll_press_count;
    uint8_t is_check_highlight;
    int32_t light_sensor_val;
    uint8_t reserve[MAX_RESERVE_SIZE];
...
typedef struct {
    ...
    uint32_t fast_auth_get_feature1_time[MAX_PT_NUM];
    uint32_t fast_auth_get_feature2_time[MAX_PT_NUM];
    uint32_t fast_auth_get_feature3_time[MAX_PT_NUM];
    ...
    int32_t auth_status;
    uint64_t align_reserve;  // used for make struct align by 8bytes
    uint8_t reserve[MAX_PT_NUM * MAX_RESERVE_SIZE];
...
typedef struct {
    ...
    uint8_t retry_count;
    gf_delmar_performance_piece_t pieces[MAX_SENSOR_NUM][MAX_SG_NUM];
    uint32_t capture_image_detail_time[4];
...
typedef struct {
    ...
    uint8_t i_is_first_part;  // Only valid when dsp opened
...
typedef struct {
    ...
    uint32_t i_gid;
    uint32_t i_finger_id;
    uint16_t i_temp_flag;
    uint16_t i_samples_remaining;
    uint8_t reserve[MAX_RESERVE_SIZE];
} gf_delmar_enroll_end_t;


typedef enum {
    GF_DELMAR_CMD_UPDATE_SENSOR_IDS = 100 + 1,
    ...
    GF_DELMAR_CMD_ALGO_FAST_GET_FEATURE_ONE,
    GF_DELMAR_CMD_ALGO_FAST_GET_FEATURE_THREE,
    GF_DELMAR_CMD_ALGO_ENROLL_END,
...
typedef struct {
    ...
    uint64_t align_reserve;  // used for make struct align by 8bytes
    float    hb_expo_time;
    uint16_t hb_expo_gain;
    float    hb_hl_expo_time;
    uint8_t reserve[MAX_RESERVE_SIZE - 12];
...
typedef struct {
    uint16_t expo_time;
    uint16_t long_expo_time;
    uint16_t hb_expo_time;
    uint16_t lb_expo_time;
    uint16_t lb_lexpo_time;
    uint64_t align_reserve;  // used for make struct align by 8bytes
    uint8_t reserve[MAX_RESERVE_SIZE];
} gf_delmar_exposure_time_t;
