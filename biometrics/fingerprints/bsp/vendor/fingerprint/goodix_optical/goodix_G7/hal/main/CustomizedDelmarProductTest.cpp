...
CustomizedDelmarProductTest::CustomizedDelmarProductTest(HalContext *context)
    ...
    LOG_D(LOG_TAG, "[%s] mProductScreenId=%d, sensorType=%d, opticalType=%d, sensorBgVersion=%d",
          __func__,
          mProductScreenId, sensorType, opticalType, sensorBgVersion);
    if (sensorType == DELMAR_SENSOR_TYPE_SINGLE_T_SE2
            || sensorType == DELMAR_SENSOR_TYPE_SINGLE_T_SE5
            || sensorType == DELMAR_SENSOR_TYPE_SINGLE_CS_SE3) {
...
void CustomizedDelmarProductTest::setThresholdNewFlow(uint8_t opticalType, uint32_t bgVersion) {
    ...
    if (sensorType == DELMAR_SENSOR_TYPE_SINGLE_CS_SE3) {
        // g7
        uint8_t cChipType = ((DelmarSensor *) mContext->mSensor)->getCChipType();
        LOG_D(LOG_TAG, "[%s] set g7 product threshold, c chip type = %d", __func__, cChipType);
        mNewTestConfig.maxBadPixelNum = 130;
        mNewTestConfig.maxBadBlockNum = 8;
        mNewTestConfig.maxBadBlockLargest = 60;
        mNewTestConfig.maxBadPixelAllBlocks = 75;
        mNewTestConfig.maxBadLineNum = 0;
        mNewTestConfig.maxHotPixelLowNum = 93;
        ...
        if (cChipType == DELMAR_C_CHIP_TYPE_GW183) {
            mNewTestConfig.maxDPSNoiseDarkN = 10.20;
                mNewTestConfig.maxSNoiseDarkN = 8.5;
                mNewTestConfig.maxLightLeakDark = 1000;  // not check
        } else {
            mNewTestConfig.maxDPSNoiseDarkN = 6.5;
                mNewTestConfig.maxSNoiseDarkN = 6;
                mNewTestConfig.maxLightLeakDark = 45;
        }
        ...
        mNewTestConfig.maxTNoiseDarkN = 5;
        mNewTestConfig.maxTNoiseLightN = 32;
        mNewTestConfig.maxSNoiseLightN = 120;
        mNewTestConfig.minLightHighMean = 1000;
        mNewTestConfig.maxLightHighMean = 3000;
        mNewTestConfig.maxLightLeakRatio = 0.85;
        mNewTestConfig.minPolarDegree = 1.28;
        mNewTestConfig.minSignalLLow = 31.57;
        mNewTestConfig.minSignalLHigh = 53.04;
        mNewTestConfig.maxSNoiseFlatL = 10;
        mNewTestConfig.minTSNRLow = 13.30;
        mNewTestConfig.minTSNRHigh = 22.87;
        mNewTestConfig.minSharpnessL = 0.0;  // not check
        mNewTestConfig.maxAssemblyAngleOffset = 2;
        mNewTestConfig.maxAssemblyXOffset = 12;
        mNewTestConfig.maxAssemblyYOffset = 12;
        ...
        mNewTestConfig.minDiffBlackLD = -10000;  // g6x not check this, set so small for not check
        mNewTestConfig.maxDiffOffset = 1500;
        mNewTestConfig.maxLightStability = 50;
        ...
        mNewTestConfig.maxChartDirty = 1000;  // not check this for g6x, set it big
        mNewTestConfig.standardAngle = 101.0; // 11 degree rotate
        mNewTestConfig.standardCenterX = 50;
        mNewTestConfig.standardCenterY = 50;
        mNewTestConfig.maxHRDNoise = 1000.0;  // not check this, set it big
        // mNewTestConfig.maxFlesh3DFool = 1000; //  not check, set so big for not check
        mNewTestConfig.maxDiffRatioForDecExpo = 0.025;
    } else {
        if (opticalType == DELMAR_OPTICAL_TYPE_3_0) {
            ...
            mNewTestConfig.maxHotLineNum = 1000;
            mNewTestConfig.maxHotBlockLargest = 17;
            mNewTestConfig.maxDPBadPixelNum = 50;
            mNewTestConfig.maxDPBadPixelInRow = 8;
...
gf_error_t CustomizedDelmarProductTest::checkGapValueOfEachBrightness(
...
gf_error_t CustomizedDelmarProductTest::checkStabilityOfSwitchingBrightness(
...
gf_error_t CustomizedDelmarProductTest::checkChartDirection(gf_delmar_calculate_cmd_t *cal_cmd,
        int32_t sensorNum) {
    gf_error_t err = GF_SUCCESS;
    ...
    case SCREEN_TYPE_CC161_SAMSUNG_ID: {
        for (sensor_index = 0; sensor_index < sensorNum; sensor_index++) {
            ...
            LOG_D(LOG_TAG, "[%s] sensor_index =%d, directionOffset=%d", __func__, sensor_index,
                  cal_cmd->o_result.nDirectionOffset[sensor_index]);
            if (directionOffset > mTestConfig.maxChartDirection) {
                ...
                LOG_E(LOG_TAG, "[%s] sensor_index =%d, directionOffset=%d(<=%d)", __func__, sensor_index,
                      directionOffset, mTestConfig.maxChartDirection);
            } else {
...
gf_error_t CustomizedDelmarProductTest::checkPerformanceThreshold(gf_calculate_cmd_result_t
        *cal_result) {
    gf_error_t err = GF_SUCCESS;
    FUNC_ENTER();
    LOG_D(LOG_TAG, "[%s] mProductScreenId=%d", __func__, mProductScreenId);
...
gf_error_t CustomizedDelmarProductTest::checkChipOffsetAngel(gf_delmar_calculate_cmd_t *cal_cmd,
        int32_t sensorNum) {
    gf_error_t err = GF_SUCCESS;
    FUNC_ENTER();
    LOG_D(LOG_TAG, "[%s] mProductScreenId=%d", __func__, mProductScreenId);
...
gf_error_t CustomizedDelmarProductTest::checkChipOffsetCoordinate(gf_delmar_calculate_cmd_t
        *cal_cmd, int32_t sensorNum) {
    gf_error_t err = GF_SUCCESS;
    FUNC_ENTER();
    LOG_D(LOG_TAG, "[%s] mProductScreenId=%d", __func__, mProductScreenId);
    switch (mProductScreenId) {
...
gf_error_t CustomizedDelmarProductTest::checkBadPoint(gf_delmar_calculate_cmd_t *cal_cmd,
        int32_t sensorNum) {
    gf_error_t err = GF_SUCCESS;
    FUNC_ENTER();
    LOG_D(LOG_TAG, "[%s] mProductScreenId=%d", __func__, mProductScreenId);
...
gf_error_t CustomizedDelmarProductTest::saveOldPerformanceTestResult(char *timeStamp,
        gf_delmar_calculate_cmd_t *cal_cmd, uint32_t operationStep, uint8_t sensorNum,
        gf_delmar_product_test_config_t *threshold) {
    gf_error_t err = GF_SUCCESS;
        ...
        if (OPERATION_STEP_CALCULATE_SIMPLIFIED_PERFORMANCE_INDICATORS == operationStep) {
            ...
            */
            ...
            if (buffer.size() > 0) {
                ...
                snprintf(file_name, GF_DUMP_FILE_PATH_MAX_LEN, "%s_err%05d_result_threshold.csv", timeStamp,
                         cal_cmd->err_code);
...
gf_error_t CustomizedDelmarProductTest::saveNewPerformanceTestResult(char *timeStamp,
        gf_delmar_calculate_cmd_t *cal_cmd, uint32_t operationStep, uint8_t sensorNum,
        gf_delmar_product_test_config_t *threshold_old) {
    gf_error_t err = GF_SUCCESS;
    char file_path[GF_DUMP_FILE_PATH_MAX_LEN] = {0};
    uint8_t opticalType = ((DelmarSensor *) mContext->mSensor)->getOpticalType();
    uint32_t sensorIdLen = ((DelmarSensor *) mContext->mSensor)->getSensorIdLen();
    Vector<uint8_t> buffer;
    ...
    do {
        ...
        if (OPERATION_STEP_CALCULATE_SIMPLIFIED_PERFORMANCE_INDICATORS == operationStep) {
            ...
            strncat(line, "SensorID,", strlen("SensorID,"));
            for (int id = 0; id < (int32_t) sensorNum; id++) {
                char sensor_id[DELMAR_SENSOR_ID_MAX_BUFFER_LEN * 2 + 1] = { 0 };
                for (int i = 0; i < (int32_t)sensorIdLen; i++) {
                    char subId[3] = { 0 };
                    snprintf(subId, sizeof(subId), "%02X", result->nSensorID[id][i]);
                    strncat(sensor_id, subId, sizeof(subId));
                }
            ...
            buffer.appendArray((uint8_t *) line, strlen(line));
            if (!DelmarHalUtils::isOptical7X(opticalType)) {
                // HAF
                // HAFBadPointNum
                PRINT_NUMBER_ARRAY_TO_BUFFER_THRESHOLD(line, sizeof(line), "HAFBadPointNum", "%d",
            ...
            if (!DelmarHalUtils::isOptical7X(opticalType)) {
                PRINT_NUMBER_ARRAY_TO_BUFFER_THRESHOLD(line, sizeof(line), "UnorSignal_LPF", "%.4f",
                                                       result->nSignalL, ">=%f", threshold->minSignalL, false, 256.0);
                buffer.appendArray((uint8_t *) line, strlen(line));
            } else {
                ...
                buffer.appendArray((uint8_t *) line, strlen(line));
                PRINT_NUMBER_ARRAY_TO_BUFFER_THRESHOLD(line, sizeof(line), "SignalLHigh", "%.4f",
            ...
            buffer.appendArray((uint8_t *) line, strlen(line));
            // MinDiffBlackLD
            PRINT_NUMBER_ARRAY_TO_BUFFER_THRESHOLD(line, sizeof(line), "MinDiffBlackLD", "%d",
                                                   result->nDiffBlackLD, ">=%d", threshold->minDiffBlackLD, false, 1);
            buffer.appendArray((uint8_t *) line, strlen(line));
            // MaxDiffOffset
            PRINT_NUMBER_ARRAY_TO_BUFFER_THRESHOLD(line, sizeof(line), "MaxDiffOffset", "%d",
            ...
            buffer.appendArray((uint8_t *) line, strlen(line));
            // HeartRateNoise
            PRINT_NUMBER_ARRAY_TO_BUFFER_THRESHOLD(line, sizeof(line), "HRDNoise", "%.4f",
                result->nHRDNoise, "<=%f", threshold->maxHRDNoise, false, 1);
            buffer.appendArray((uint8_t *) line, strlen(line));
            /*
            // ChartGhostShadow
            ...
            buffer.appendArray((uint8_t *) line, strlen(line));
            // light jitter
            PRINT_NUMBER_ARRAY_TO_BUFFER_THRESHOLD(line, sizeof(line), "LightJitter", "%f",
                    result->nLightJitter, "<=%d", threshold->maxLightJitter, false, 1.0);
            buffer.appendArray((uint8_t *) line, strlen(line));
            */
            if (!DelmarHalUtils::isOptical7X(opticalType)) {
                // ChartDirty
            ...
            PRINT_NUMBER_ARRAY_TO_BUFFER_THRESHOLD(line, sizeof(line), "HfleshDiffRatio", "%.3f",
                result->nHfleshDiffRatio, "<=%f", threshold->maxDiffRatioForDecExpo, false, 1.0);
            buffer.appendArray((uint8_t *) line, strlen(line));
            PRINT_NUMBER_ARRAY_TO_BUFFER_THRESHOLD(line, sizeof(line), "HblackDiffRatio", "%.3f",
                result->nHblackDiffRatio, "<=%f", threshold->maxDiffRatioForDecExpo, false, 1.0);
            buffer.appendArray((uint8_t *) line, strlen(line));
...
gf_error_t CustomizedDelmarProductTest::savePerformanceTestResult(char *timeStamp,
        gf_delmar_calculate_cmd_t *cal_cmd, uint32_t operationStep, uint8_t sensorNum,
        gf_delmar_product_test_config_t *threshold) {
    gf_error_t err = GF_SUCCESS;
    Vector<uint8_t> buffer;
    ...
    do {
        ...
        gf_delmar_sensor_type_t sensorType = ((DelmarSensor *) mContext->mSensor)->getSenosrType();
        LOG_D(LOG_TAG, "[%s] sensorIdx=%d, operationStep=%d, sensorType = %d", __func__, sensorIdx,
              operationStep, sensorType);
...
gf_error_t CustomizedDelmarProductTest::testOTPFLash() {
    ...
    gf_delmar_test_otp_cmd_t cmd = { {0} };
    ...
    do {
        ...
        cmd.header.cmd_id = GF_CMD_TEST_OTP_FLASH;
        err = invokeCommand(&cmd, sizeof(gf_delmar_test_otp_cmd_t));
        if (err != GF_SUCCESS) {
