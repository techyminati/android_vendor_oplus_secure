...
#define GF_CMD_TEEC_PARAM_TYPES (TEEC_PARAM_TYPES(TEEC_MEMREF_TEMP_INOUT, TEEC_VALUE_INOUT, TEEC_MEMREF_TEMP_INOUT, TEEC_NONE))
...
namespace goodix {
    ...
    static uint64_t *gpTimeStamp = NULL;
...
#endif  // #ifdef SUPPORT_TA_LOG_DUMP_TO_CA
            gpTimeStamp = new uint64_t(0);
            if (NULL == gpTimeStamp) {
                LOG_E(LOG_TAG, "[%s] malloc gpTimeStamp failed", __func__);
                err = GF_ERROR_OUT_OF_MEMORY;
                break;
            }
        } while (0);
    ...
    gf_error_t CaEntry::sendCommand(void *cmd, uint32_t size) {
        ...
        do {
            ...
            *gpTimeStamp = (((currentTm.tm_hour * 60 + currentTm.tm_min) * 60)
                                    + currentTm.tm_sec) * 1000 + time.tv_usec / 1000;
            operation.params[2].tmpref.buffer = gpTimeStamp;
            operation.params[2].tmpref.size = sizeof(uint64_t);
#ifdef SUPPORT_TA_LOG_DUMP_TO_CA
            ...
            LOG_D(LOG_TAG, "[%s] target: %d %s, cmd id: %d %s", __func__,
                header->target, module_to_str(header->target),
                header->cmd_id, cmd_to_str(header->target, header->cmd_id));
