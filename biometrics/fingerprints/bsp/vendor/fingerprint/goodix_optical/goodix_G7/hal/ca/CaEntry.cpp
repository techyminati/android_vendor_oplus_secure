 ...
 ** --------------------------- Revision History: --------------------------------
 **  <who>            <when>            <what>
 **  Bangxiong.Wu     2019/10/22        add TA load retry time to 5
 ***********************************************************************************/
...
gf_error_t CaEntry::startTap(bool isDSPEnabled) {
    ...
    for (uint32_t i = 0; i < qcontext->customizedParams.goodixTaPathCount; i++) {
        for (uint32_t j = 0; j < OPLUS_TA_LOAD_RETRY_TIMES; j++) {
            ...
            usleep(100 * 1000);//sleep 100ms for next load ta retry
            LOG_E(LOG_TAG, "[%s] Load ta: %s from path[%s] failed %d times, ret=%d, errno=%d.", __func__,
                  qcontext->customizedParams.goodixTaName, qcontext->customizedParams.goodixTaNamePaths[i], j, ret,
                  errno);
        }
        if (ret == 0) {
            err = GF_SUCCESS;
    ...
    if (err == GF_SUCCESS) {
        ...
        LOG_D(LOG_TAG, "[%s] Load ta: %s succeeded.", __func__,
              qcontext->customizedParams.goodixTaName);
        sendHmacKey(this);
...
gf_error_t CaEntry::sendCommand(void *cmd, uint32_t len) {
    ...
    do {
        ...
        qseeCmd->timestamp = (((currentTm.tm_hour * 60 + currentTm.tm_min) * 60) + currentTm.tm_sec) * 1000 + time.tv_usec / 1000;
