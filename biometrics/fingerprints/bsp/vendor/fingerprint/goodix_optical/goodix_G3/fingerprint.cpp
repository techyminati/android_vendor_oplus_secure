        ...
        uint32_t  gid, uint32_t authtype)
{
    ...
    (void)dev;
    return gpHalContext->mFingerprintCore->authenticateAsType(operation_id, gid, authtype);
}

int fingeprrint_set_image_dump_flag(struct fingerprint_device *dev, uint32_t cmd, uint32_t data)
{
    (void)dev;
    (void)cmd;
    //user version cannot set dump_flag to 1.
    //gpHalContext->mFingerprintCore->setImageDumpFlag(data);
    return 0;
}
...
static int32_t open(const hw_module_t *pModule, const char __unused *pId, hw_device_t **ppDevice)
{
    ...
    pDev->set_image_dump_flag = fingeprrint_set_image_dump_flag;
    /*oplus api end*/
