...
namespace goodix
{
    ...
    protected:
        ...
        virtual gf_error_t onEnrollRequested(const MsgBus::Message &msg);
        ...
        virtual gf_error_t getDumpData(gf_data_info_t* cmd, uint8_t** data, uint32_t* data_len);

        virtual gf_error_t dumpDeviceInfo(char* timestamp, uint8_t* data, uint32_t data_len, bool is_dump_init_info = false);
        virtual gf_error_t dumpTemplates(char* timestamp, uint8_t* data, uint32_t data_len);
        ...
        virtual gf_error_t onNotifyAuthSuccessEnd(const MsgBus::Message& msg);
    ...
    protected:
        Vector<AsyncMessage*> mDumpQueue;
    };
