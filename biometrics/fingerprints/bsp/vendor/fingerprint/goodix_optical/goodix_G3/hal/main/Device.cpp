...
namespace goodix
{
    ...
    enum NETLINK_CMD
    {
        GF_NETLINK_TEST = 0,  //
        GF_NETLINK_IRQ = 1,
        GF_NETLINK_SCREEN_OFF = 2,
        GF_NETLINK_SCREEN_ON = 3,
        GF_NETLINK_TP_TOUCHDOWN = 4,
        GF_NETLINK_TP_TOUCHUP = 5,
        GF_NETLINK_UI_READY = 6,
    ...
    Device::Device(HalContext* context) :
            HalBase(context), mFD(-1), mStopWaitInterrupt(false), mSpiSpeed(GF_SPI_SPEED_LOW), mNetlinkRoute(
    ...
    gf_error_t Device::stop_wait_fingerprint_event()
    {
        gf_error_t err = GF_SUCCESS;
        if (mFD < 0) {
            LOG_E(LOG_TAG, "[%s], fd not exist, %s", __func__, GF_DEV_NAME);
            err = GF_ERROR_HAL_FILE_DESCRIPTION_NULL;
            return err;
        }

        if (ioctl(mFD, GF_IOC_STOP_WAIT_INTERRUPT_EVENT) != 0) {
            LOG_E(LOG_TAG, "[%s] ioctl failed", __func__);
            err = GF_ERROR_HAL_IOCTL_FAILED;
            return err;
        }
                ...
                || GF_NETLINK_TP_TOUCHUP == msg || GF_NETLINK_UI_READY == msg)
        {
            ...
            else if (GF_NETLINK_TP_TOUCHUP == msg)
            {
                ...
                if (nullptr != mContext && nullptr != mContext->mFingerprintCore) {
                    ...
                    LOG_E(LOG_TAG, "[%s] notify EVENT_FINGER_UP fail", __func__);
                }
            }
    ...
    {
        if (mDevice) {
            mDevice->stop_wait_fingerprint_event();
            mDevice->mStopWaitInterrupt = true;
        }
        mDevice = NULL;
    }
    void Device::NetlinkRoutingThread::interruptProcessFpEvent()
    {
        gf_error_t err = GF_SUCCESS;
        FUNC_ENTER();
        if (mDevice->mFD < 0) {
        ...
        struct fingerprint_message_t msg = {0};
        while (1) {
            if (mDevice->mStopWaitInterrupt) {
                break;
            }
            memset(&msg, 0 , sizeof(msg));
            LOG_E(LOG_TAG, "start wait event\n");
            if (read(mDevice->mFD, &msg, sizeof(msg)) == sizeof(msg)) {
                LOG_E(LOG_TAG, "module:%d, event:%d\n", msg.module, msg.event);
                if (msg.module == E_FP_HAL && msg.event == E_FP_EVENT_STOP_INTERRUPT) {
                    break;
                }
                mDevice->handleNetlinkMessage(msg.event);
            }
        }
    ...
    void Device::NetlinkRoutingThread::netlinkProcessFpEvent()
    {
    ...
    {
        int fd = ::open("/proc/fp_kernel_event", O_RDWR);
        int value = FP_DIRVER_NETLINK;
        int len = 0;
        if (fd < 0) {
            goto fp_out;
        }
        len = read(fd, &value, sizeof(int));
        ::close(fd);
        if (len != sizeof(int)) {
            value = FP_DIRVER_NETLINK;
            goto fp_out;
        }
    fp_out:
        LOG_I(LOG_TAG, "read fp kernel type value:%d", value);
        if (value == FP_DIRVER_NETLINK) {
            netlinkProcessFpEvent();
        } else if (value == FP_DRIVER_INTERRUPT) {
            interruptProcessFpEvent();
        } else {
            printf("kernel event not define");
        }
        return false;
    }
