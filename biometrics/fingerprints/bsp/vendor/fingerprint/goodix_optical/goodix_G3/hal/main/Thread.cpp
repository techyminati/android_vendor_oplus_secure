...
namespace goodix
{
    ...
    static bool createThread(android_pthread_entry entryFunction, void* userData, const char* name,
            int32_t priority, uint32_t stack, uint32_t* threadId)
    {
        ...
        if (result != 0)
        {
            LOG_E(LOG_TAG, "[%s] create thread failed, result = %d, priority = %d, stack = %d.",
