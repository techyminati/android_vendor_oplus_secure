                             ...
                             UREE_SESSION_HANDLE *pHandle, const char *tag)
{
    ...
    memset(&param, 0, sizeof(struct kree_session_cmd_param));
  ...
  const char *tag, uint32_t readOnly)
{
    ...
    *shm_handle = 0; /* 0: invalid handle */
    ...
    ALOGI ("[%s] ====> p.param.size=%x, param->buffer=0x%x  \n", __FUNCTION__, p.param.size, (uint32_t) p.param.buffer);
...
TZ_RESULT UREE_RegisterSharedmemWithTag(UREE_SESSION_HANDLE handle,
	UREE_SHAREDMEM_HANDLE *shm_handle, UREE_SHAREDMEM_PARAM *param, const char *tag)
{
...
TZ_RESULT UREE_RegisterSharedmem_ReadOnlyWithTag(UREE_SESSION_HANDLE handle,
  UREE_SHAREDMEM_HANDLE *shm_handle, UREE_SHAREDMEM_PARAM *param, const char *tag)
{
								   ...
								   UREE_SHAREDMEM_HANDLE shm_handle)
{
    ...
    ret = UREE_TeeServiceCall(handle, TZCMD_MEM_SHAREDMEM_UNREG, TZ_ParamTypes1(TZPT_VALUE_INPUT), p);
    if (ret!= TZ_RESULT_SUCCESS) {
        ALOGE ("[%s] ====> UREE_TeeServiceCall():echo_session[0x5588] returns Fail. [Stop!]. ret=0x%x \n", __FUNCTION__, ret);
        return ret;
    }
...
TZ_RESULT UREE_ION_CP_Chm2Shm(UREE_SESSION_HANDLE session,
						UREE_SHAREDMEM_HANDLE shm_handle,
						UREE_ION_HANDLE ION_Handle, uint32_t size)
{
...
TZ_RESULT UREE_ION_TO_SHM_HANDLE(UREE_ION_HANDLE ION_Handle, UREE_SHAREDMEM_HANDLE *shm_handle)
{
	struct user_sc_param p;
	int iret;
