...
namespace goodix
{
    ...
    BUFFER_INFO* IonMemory::allocate(int32_t size)
    {
        ...
        do
        {
            ...
            if (mIonDeviceFd == 0 || mpCachedInfo == NULL)
            {
                err = init();
                if (err != GF_SUCCESS)
                {
        ...
        do {
            if (0 == mCarveoutIonHandleFd) {
                ...
                if (retval) {
                    ...
                    LOG_E("[%s] qsc_ion_carveout_memalloc failed with retval = %d\n", __func__, retval);
                    break;
                }
            }
        } while (0);
    ...
    gf_error_t IonMemory::doAllocate(int32_t size, BUFFER_INFO *info)
    {
        ...
        if (ret)
        {
            LOG_E(LOG_TAG, "[%s] Error::ion_alloc_fd for heap %u size %u len %u failed ret = %d, errno = %d\n", __func__, heap_id, size, len, ret, errno);
            err = GF_ERROR_GENERIC;
            goto alloc_fail;
        }
        ...
        pExtInfo->bufferHandle  =  mIonDeviceFd;  // ion device fd
        ...
        if (ret)
        {
            LOG_E(LOG_TAG, "[%s] Error:: DMA_BUF_IOCTL_SYNC start failed, ret = %d, errno = %d\n", __func__, ret, errno);
            err = GF_ERROR_GENERIC;
            goto sync_fail;
        }
        return err;
...
alloc_fail:
    ...
    gf_error_t IonMemory::doFree(BUFFER_INFO *info)
    {
        ...
        if (pExtInfo->bufferHandle > 0)   // ion device fd
        {
            ion_close(pExtInfo->bufferHandle);
        }
    ...
    gf_error_t IonMemory::doAllocate(int32_t size, BUFFER_INFO *info)
    {
        ...
        memset(&allocData, 0, sizeof(ion_allocation_data));

        struct ion_fd_data fdData;
        memset(&fdData, 0, sizeof(ion_fd_data));
