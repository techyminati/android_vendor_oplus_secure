...
namespace goodix
{
    ...
    gf_error_t SZFingerprintCore::onAfterAuthAlgo(AuthenticateContext *context)
    {
        ...
        if (context->result == GF_ERROR_NOT_MATCH)
        {
            ...
            else
            {
                if (context->retry != 0)
                {
                    ...
                    context->result = mContext->mSensor->captureImage(GF_OPERATION_AUTHENTICATE, context->retry + 1);
            ...
            LOG_E(LOG_TAG, "[%s] antipeep & screen struct flag : %d", __func__, context->auth_cmd->o_antipeep_screen_struct_flag);
        }
        // if dump propperty is seted, use for dump data
        sendMessage(MsgBus::MSG_AUTHENTICATE_ALGO_END, context->result, context->retry);

        LOG_D(LOG_TAG, "[%s] up %d, plus %d", __func__, context->up, continue_plus);
        ...
        if (context->result == GF_ERROR_NOT_MATCH &&
                    continue_plus == 1 && context->retry < 2)
        {
            context->auth_cmd->i_retry_count = context->retry + 1;
            context->auth_cmd->i_recog_round = 0;
            context->result = mContext->mAlgo->authImage(context->auth_cmd);
            LOG_D(LOG_TAG, "[%s] antipeep & screen struct flag : %d", __func__, context->auth_cmd->o_antipeep_screen_struct_flag);
            // if dump propperty is seted, use for dump data
            sendMessage(MsgBus::MSG_AUTHENTICATE_ALGO_END, context->result, context->retry + 1);
        }
