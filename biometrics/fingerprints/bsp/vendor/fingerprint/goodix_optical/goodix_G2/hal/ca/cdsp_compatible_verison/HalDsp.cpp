 ...
 **  Ran.Chen   2020/03/25         modify for cdsp compatbile
 ************************************************************************************/
...
namespace goodix {
    ...
    HalDsp::HalDsp(CaEntry *caEntry) : mDspTime(0), mDspStatus(DSP_NOT_AVAILABLE), mDspResult(0) {
        mCaEntry = caEntry;
    ...
    gf_error_t HalDsp::proxyHandleInit(void) {
        ...
        do {
            ...
            if (NULL == gf_proxy_dsp_init || NULL == gf_proxy_dsp_reinit
                ...
                || NULL == gf_proxy_send_cmd_to_dsp || NULL == gf_proxy_set_callback) {
    ...
    gf_error_t HalDsp::init() {
        ...
        do {
            ...
            ret = gf_proxy_dsp_init(fd, len, CDSP_LOG, 0);
    ...
    gf_error_t HalDsp::reinit() {
        ...
        do {
            ...
            ret = gf_proxy_dsp_reinit(fd, len, CDSP_LOG, 0);
            if (ret) {
