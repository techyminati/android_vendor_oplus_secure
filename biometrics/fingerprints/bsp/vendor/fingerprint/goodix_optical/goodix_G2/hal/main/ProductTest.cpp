...
namespace goodix
{
    ...
    ProductTest::ProductTest(HalContext* context)
        : HalBase(context), mpNotify(nullptr)
    {  // NOLINT(432)
    ...
    gf_error_t ProductTest::onCommand(int32_t cmdId, const int8_t *in, uint32_t inLen, int8_t **out, uint32_t *outLen)
    {
    ...
    gf_error_t ProductTest::executeCommand(int32_t cmdId, const int8_t *in, uint32_t inLen, int8_t **out, uint32_t *outLen)
    {
        gf_error_t err = GF_SUCCESS;
        UNUSED_VAR(in);
        UNUSED_VAR(inLen);
    ...
    void ProductTest::setNotify(product_test_callback notify)
    {
        AutoMutex _l(mContext->mHalLock);
        mpNotify = notify;
    }
    ...
    void ProductTest::notifyTestCmd(int64_t devId, int32_t cmdId, const int8_t *result, int32_t resultLen)
    {
        VOID_FUNC_ENTER();
        if (mpNotify != NULL)
        {
