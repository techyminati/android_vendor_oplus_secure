...
//Algo version
#define GOODIX_ALGO_VERSION "v03.02.01.03"
...
namespace goodix {
    ...
    const char* CustomizedFingerprintCore::getDisableStudyProperty() {
        return NULL;
    }
    ...
    gf_error_t CustomizedFingerprintCore::getAlgoVersion() {
        ...
        do {
            ...
            if(0 == strcmp(mALGO_VERSION, "")) {
                LOG_D(LOG_TAG, "[%s] get AlgoVersion from ta is null", __func__);
                memcpy(mALGO_VERSION, GOODIX_ALGO_VERSION, sizeof(GOODIX_ALGO_VERSION));
            }
        } while (0);
    ...
    gf_error_t CustomizedFingerprintCore::send_auth_dcsmsg(AuthenticateContext* context, bool notified) {
        ...
        if (mNotify != nullptr) {
            ...
            message.data.auth_dcsmsg.img_area = detail.o_valid_area;//invalid
            ...
            message.data.auth_dcsmsg.dsp_availalbe = 1;//invalid
