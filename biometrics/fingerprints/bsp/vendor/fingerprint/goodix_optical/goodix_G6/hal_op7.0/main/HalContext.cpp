...
namespace goodix {
    ...
    gf_error_t HalContext::init() {
        ...
        do {
            ...
            GF_ERROR_BREAK(err);
#ifdef SUPPORT_PERSIST_DATA_BACKUP
            mDataSynchronizer = new DataSynchronizer(this);
            err = mDataSynchronizer->init();
            GF_ERROR_BREAK(err);
#endif  // SUPPORT_PERSIST_DATA_BACKUP
#ifdef SUPPORT_DSP_HAL
            // init dsp
...
#ifdef SUPPORT_PERSIST_DATA_BACKUP
        if (mDataSynchronizer != nullptr) {
            delete mDataSynchronizer;
            mDataSynchronizer = nullptr;
        }
#endif  // SUPPORT_PERSIST_DATA_BACKUP
...
#ifdef SUPPORT_PERSIST_DATA_BACKUP
            if (header->persist_data_change > 0) {
                LOG_D(LOG_TAG, "[%s] persist data change", __func__);
                mMsgBus.sendMessage(MsgBus::MSG_PERSIST_DATA_CHANGE);
                header->persist_data_change = 0;
            }
#endif  // SUPPORT_PERSIST_DATA_BACKUP
        }
        while (0);
