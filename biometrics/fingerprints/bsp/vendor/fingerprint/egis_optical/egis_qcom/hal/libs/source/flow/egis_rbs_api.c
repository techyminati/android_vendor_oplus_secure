...
{
	ex_log(LOG_DEBUG, "#*rbs_initialize#*#*[enter]#*");
	ex_log(LOG_DEBUG, "GIT_SHA1 = br_ipp6_pre-verify_2e89c8c");
	...
	ex_log(LOG_DEBUG, "#*rbs_initialize#*#*[leave] %d#*", retval);
	return retval;
}

int rbs_uninitialize()
{
	ex_log(LOG_DEBUG, "#*rbs_uninitialize#*#*[enter]#*");

	int retval = cpt_uninitialize();

	ex_log(LOG_DEBUG, "#*rbs_uninitialize#*#*[leave] %d#*", retval);
	return retval;
}

int rbs_cancel()
{
	ex_log(LOG_DEBUG, "#*rbs_cancel#*#*[enter]#*");

	int retval = cpt_cancel();

	ex_log(LOG_DEBUG, "#*rbs_cancel#*#*[leave] %d#*", retval);
	return retval;
}

int rbs_active_user_group(unsigned int user_id, const char *data_path)
{
	ex_log(LOG_DEBUG, "#*rbs_active_user_group#*#*[enter]#*");
	...
	ex_log(LOG_DEBUG, "#*rbs_active_user_group#*#*[leave] %d#*", retval);
	return retval;
}
...
{
	ex_log(LOG_DEBUG, "#*rbs_set_data_path#*#*[enter]#*");
	...
	ex_log(LOG_DEBUG, "#*rbs_set_data_path#*#*[leave] %d#*", retval);
	return retval;
}

int rbs_chk_secure_id(unsigned int user_id, unsigned long long sid)
{
	ex_log(LOG_DEBUG, "#*rbs_chk_secure_id#*#*[enter]#*");

	int retval = cpt_chk_secure_id(user_id, sid);

	ex_log(LOG_DEBUG, "#*rbs_chk_secure_id#*#*[leave] %d#*", retval);
	return retval;
}

int rbs_chk_auth_token(unsigned char *token, unsigned int len)
{
	ex_log(LOG_DEBUG, "#*rbs_chk_auth_token#*#*[enter]#*");
	...
	ex_log(LOG_DEBUG, "#*rbs_chk_auth_token#*#*[leave] %d#*", retval);
	return retval;
}

int rbs_pre_enroll(unsigned int user_id, unsigned int finger_id)
{
	ex_log(LOG_DEBUG, "#*rbs_pre_enroll#*#*[enter]#*");
	ex_log(LOG_DEBUG, "commander_version: %s", COMMANDER_VERSION);
	ex_log(LOG_DEBUG, "GIT_SHA1 = br_ipp6_pre-verify_2e89c8c");
	...
	ex_log(LOG_DEBUG, "#*rbs_pre_enroll#*#*[leave] %d#*", retval);
	return retval;
}

int rbs_enroll()
{
	ex_log(LOG_DEBUG, "#*rbs_enroll#*#*[enter]#*");

	int retval = cpt_enroll();

	ex_log(LOG_DEBUG, "#*rbs_enroll#*#*[leave] %d#*", retval);
	return retval;
}

int rbs_post_enroll()
{
	ex_log(LOG_DEBUG, "#*rbs_post_enroll#*#*[enter]#*");

	int retval = cpt_post_enroll();

	ex_log(LOG_DEBUG, "#*rbs_post_enroll#*#*[leave] %d#*", retval);
	return retval;
}

int rbs_get_authenticator_id(unsigned long long *id)
{
	ex_log(LOG_DEBUG, "#*rbs_get_authenticator_id#*#*[enter]#*");
	...
	ex_log(LOG_DEBUG, "#*rbs_get_authenticator_id#*#*[leave] %d#*", retval);
	return retval;
}

int rbs_authenticator(unsigned int user_id, unsigned int *finger_ids,
		      unsigned int finger_count, unsigned long long challenge)
{
	ex_log(LOG_DEBUG, "#*rbs_authenticator#*#*[enter]#*");
	ex_log(LOG_DEBUG, "GIT_SHA1 = br_ipp6_pre-verify_2e89c8c");
	...
	if (finger_ids) {
		memcpy(verify_info.fingerprints.fingerprint_ids, finger_ids,
	...
	ex_log(LOG_DEBUG, "#*rbs_authenticator#*#*[leave] %d#*", retval);
...
{
	ex_log(LOG_DEBUG, "#*rbs_remove_fingerprint#*#*[enter]#*");
	...
	ex_log(LOG_DEBUG, "#*rbs_remove_fingerprint#*#*[leave] %d#*", retval);
	return retval;
}

int rbs_get_fingerprint_ids(unsigned int user_id, int *finger_ids,
			    int *finger_count)
{
	ex_log(LOG_DEBUG, "#*rbs_get_fingerprint_ids#*#*[enter]#*");
	fingerprint_ids_t fps;

	if (NULL == finger_ids || NULL == finger_count ||
	    *finger_count > __SINGLE_UPPER_LIMITS__) {
		return FINGERPRINT_RES_INVALID_PARAM;
	}

	fps.fingerprint_ids_count = *finger_count;
	memcpy(fps.fingerprint_ids, finger_ids,
	       sizeof(int) * fps.fingerprint_ids_count);

	int retval = cpt_get_fingerprint_ids(user_id, &fps);

	if (FINGERPRINT_RES_SUCCESS == retval) {
		*finger_count = fps.fingerprint_ids_count;
		memcpy(finger_ids, fps.fingerprint_ids,
		       fps.fingerprint_ids_count * sizeof(unsigned int));
	}

	ex_log(LOG_DEBUG, "#*rbs_get_fingerprint_ids#*#*[leave] %d#*", retval);
	return retval;
}

void rbs_set_on_callback_proc(operation_callback_t operation_callback)
{
	ex_log(LOG_DEBUG, "#*rbs_set_on_callback_proc#*#*[enter]#*\n");
	...
	ex_log(LOG_DEBUG, "#*rbs_set_on_callback_proc#*#*[leave] #*\n");
}

int rbs_navigation(operation_callback_t callback)
{
	ex_log(LOG_DEBUG, "#*rbs_navigation#*#*[enter]#*\n");

	if (NULL == callback) {
		return FINGERPRINT_RES_INVALID_PARAM;
	}

	cpt_set_event_callback(callback);
	...
	ex_log(LOG_DEBUG, "#*rbs_navigation#*#*[leave] #*\n");
...
{
	ex_log(LOG_DEBUG, "#*rbs_extra_api#*#*[enter]#*");

	int retval = cpt_extra_api(type, in_buffer, in_buffer_size, out_buffer,
				   out_buffer_size);

	ex_log(LOG_DEBUG, "#*rbs_extra_api#*#*[leave] %d#*", retval);
