...
struct heap_info g_heap_info = {0};
...
void SHOW_HEAP_ALLOC_INFO(void* buf, size_t size)
{
	...
	if (g_heap_info.heap_max_index == HEAP_RECORD_MAX) {
		egislog_e("heap_info buffer is full, idx=%d, 0x%x, %u", g_heap_info.heap_max_index, buf, size);
		return;
	} else if (g_heap_info.heap_max_index == HEAP_RECORD_MAX - 1) {
		egislog_e("heap_info buffer almost full. idx=%d", g_heap_info.heap_max_index);
	}
...
void sys_heap_info(void)
{
	...
	// egislog_d("heap_info --------------");
	egislog_d("heap_info = %d bytes, max_size = %d bytes",
		  g_heap_info.current_size, g_heap_info.max_size);
}
