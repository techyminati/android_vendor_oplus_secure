...
static int check_connection_open()
{
	...
	const char *default_path = "libets_teeclient_v3.so";
#ifndef __aarch64__
	const char *other_path = "/vendor/lib/hw/libets_teeclient_v3.so";
#else
	const char *other_path = "/vendor/lib64/hw/libets_teeclient_v3.so";
#endif
	...
	if (NULL == g_tlcComHandle) {
		if (!g_filehandle) {
			...
			if (NULL == g_filehandle) {
				ex_log(LOG_DEBUG, "%s, qsee_init Failedd", default_path);
				ex_log(LOG_DEBUG, "second, open file other_path = %s", other_path);
				g_filehandle = dlopen(other_path, RTLD_LAZY);
			}
		}

		if (NULL == g_filehandle) {
			ex_log(LOG_ERROR, "%s, qsee_init Failedd", other_path);
		...
		if (retval) {
			...
			ex_log(LOG_INFO, "Start RbsFpAPP app: pass");
		}
	}
		...
		unsigned char *out_data, unsigned int *out_data_len)
{
	...
	ex_log(LOG_VERBOSE, "-- msg_data->process = %d ", ((transfer_package_t*)msg_data)->process);
	ex_log(LOG_VERBOSE, "-- msg_data->command = %d ", ((transfer_package_t *)msg_data)->command);
	...
	if ((out_data_len != NULL) && (out_data != NULL) && (*out_data_len > 0)) {
		ex_log(LOG_VERBOSE, " out_data_len  = %d ", *out_data_len);
	}

	ex_log(LOG_VERBOSE, "transporter end, retval = %d ", retval);
	return retval;
}
