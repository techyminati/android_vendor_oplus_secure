...
static int _map_ini_handler(void* user, const char* section, const char* name,
	const char* value){
...
void check_ini_version(){
	if (g_ini_string != NULL){
		...
		if (version_flag != INI_VERSION_FLAG){
			egislog_e("rbs_config.ini version is %d (expected version is %d)", version_flag, INI_VERSION_FLAG);
		} else {
...
#endif
	egislog_d("%s [%s] %s => %d (x1000), default %d (x1000)", __func__, section, name, (int)(ret_value * 1000), (int)(default_value * 1000));
