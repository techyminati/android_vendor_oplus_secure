 ...
 */
map_t hashmap_new() {
    hashmap_map* m = (hashmap_map*)plat_alloc(sizeof(hashmap_map));
    if (!m) goto err;

    m->data = (hashmap_element*)plat_alloc(INITIAL_SIZE * sizeof(hashmap_element));
    if (!m->data) goto err;

    mem_set(m->data, 0, INITIAL_SIZE * sizeof(hashmap_element));
...
int hashmap_rehash(map_t in) {
    ...
    hashmap_element* temp =
        (hashmap_element*)plat_alloc(2 * m->table_size * sizeof(hashmap_element));
    if (!temp) return MAP_OMEM;

    mem_set(temp, 0, 2 * m->table_size * sizeof(hashmap_element));
    /* Update the array */
    curr = m->data;
    m->data = temp;
...
void hashmap_free(map_t in) {
    hashmap_map* m = (hashmap_map*)in;
    hashmap_element* m_data = m->data;
    int index;

    for (index = 0; index < INITIAL_SIZE; index++) {
        if (m_data->in_use == 1) {
            PLAT_FREE(m_data->data);
            PLAT_FREE(m_data->key);
        }
        m_data++;
    }
