 ...
 **
 ** Description:
 **      egis_call_driver for egis optical fingerprint
 **
 ** Version: 1.0
 ** Date : 16:07:20,30/12/2019
 ** Author: zhouqijia@BSP.Fingerprint.Basic
 ** TAG: BSP.Fingerprint.Basic
 ** --------------------------- Revision History: --------------------------------
 **  <author>       <data>            <desc>
 **  Qijia.Zhou 2019/12/30           add for FP_CLEAN_TOUCH_FLAG
 ************************************************************************************/
...
unsigned int fp_device_reset_set(__unused int device_handle, __unused int high_low) {
    ...
    return egis_call_driver_ZQ((int)device_handle, FP_SENSOR_RESET_SET, (void*)pin_level, 1, NULL,
                               NULL, 0);
#else
    return FINGERPRINT_RES_SUCCESS;
#endif
...
unsigned int fp_device_reset_set_low(int device_handle) {
    if (0 == device_handle) {
        return FINGERPRINT_RES_INVALID_PARAM;
    }

    return egis_call_driver((int)device_handle, FP_POWER_ON_RESET, NULL, 0, NULL, NULL, 0);
}

unsigned int fp_device_clear_touch_flag(int device_handle) {
    if (0 == device_handle) {
        return FINGERPRINT_RES_INVALID_PARAM;
    }

    return egis_call_driver((int)device_handle, FP_CLEAN_TOUCH_FLAG, NULL, 0, NULL, NULL, 0);
}

unsigned int fp_device_poweron_reset(int device_handle)
{
#ifdef SIMULATE_NO_SENSOR
    return FINGERPRINT_RES_SUCCESS;
#endif
    ...
    return egis_call_driver((int)device_handle, FP_POWER_ON_RESET, NULL, 0,
                            NULL, NULL, 0);
}
...
unsigned int fp_device_spipin_enable(__unused int device_handle, BOOL enable)
{
    ex_log(LOG_DEBUG, "fp_device_spipin_enable enable:%d", enable);
    if (enable == TRUE) {
        return egis_call_driver((int)device_handle, FP_SPIPIN_SETTING, NULL, 0, NULL, NULL, 0);
    } else {
        return egis_call_driver((int)device_handle, FP_SPIPIN_PULLLOW, NULL, 0, NULL, NULL, 0);
    }
}
...
unsigned int fp_device_power_control(__unused int device_handle, __unused BOOL enable) {
    ...
    return egis_call_driver_ZQ((int)device_handle, FP_POWER_CONTROL, (void*)power_mode, 1, NULL,
                               NULL, 0);
#else
    return egis_call_driver((int)device_handle, FP_POWER_CONTROL, (void*)power_mode, 1, NULL, NULL,
                            0);
#endif
