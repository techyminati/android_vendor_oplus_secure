...
#define ENROLLMENT_TOTAL_TIMES VENDOR_ET7XX_ENROLL_COUNTS
...
extern unsigned int fp_device_clear_touch_flag(int device_handle);
...
{
    egislog_d("on_sync_templates fingerIds = %d, fingerIds_size = %d, groupId = %d", (int)fingerIds, fingerIds_size, groupId);
...
static void on_send_dcsmsg_msg(int32_t cmdId, int8_t* result, int result_size)
{
    egislog_i("on_send_dcsmsg_msg cmdId = %d, resultLen = %d", cmdId, result_size);

    g_dev->callback->on_send_dcsmsg(g_dev->callback_context, cmdId, result, (uint32_t)result_size);
}
...
int fingerprint_get_engineering_info(uint32_t type)
{
    ...
    switch (type)
    {
        ...
        case 2:
        case 3: //  FINGERPRINT_SELF_TEST
            pid = PID_7XX_INLINETOOL;// 10
...
{
    egislog_i("fingerprint_sendFingerprintCmd enter, type(cmd id) = %d", type);
    unsigned char in_data[sizeof(int) * 2];
    unsigned char out_data[64];
    int out_data_len = 64;
               ...
               uint32_t __unused timeout_sec)
{
    ...
    egis_tp_enable();
    fp_device_clear_touch_flag(g_hdev);
...
int fingerprint_cancel()
{
    ...
    egislog_i("fingerprint_cancel enter11");
               ...
               uint32_t fid)
{
    ...
    if (0 == fid) {
        ...
        if (FINGERPRINT_RES_SUCCESS != retval) {
            egislog_e("rbs_get_fingerprint_ids fail");
        }
    }
    ...
    if (FINGERPRINT_RES_SUCCESS == retval && 0 == fid) {
        ...
        egislog_d("fingerprint_remove return = %d, fid = %d", retval, fid);
        // how many more templates must be deleted by this operation.
        if (FINGERPRINT_RES_SUCCESS == retval){
            g_dev->callback->on_removed(g_dev->callback_context, fid, gid, 0);
        } else {
            on_error_msg(FINGERPRINT_ERROR_UNABLE_TO_PROCESS);
        }
          ...
          __unused unsigned char *buffer, __unused int buffer_size)
{
    ...
    switch (event_id) {
        ...
        case EVENT_VERIFY_NOT_MATCHED: {
            ...
            if (sizeof(hw_auth_token_t) == buffer_size) {
                ...
                egislog_e(
                    ...
                    ", buffer_size = %d",
                    (unsigned int)sizeof(hw_auth_token_t), buffer_size);
            }
        ...
        case EVENT_IMG_BAD_QLTY: {
            ...
            on_acquired_msg(FINGERPRINT_ACQUIRED_IMAGER_DIRTY);
        } break;

        case EVENT_IMG_DIRTY_SCREEN: {
            on_acquired_msg(FINGERPRINT_ACQUIRED_IMAGER_DIRTY);
        } break;
        ...
        case EVENT_SENSOR_OPTICAL_SELF_TEST: {
            p_eng_info = buffer;
#if 0
            key[0] = 0;
            ...
            on_engineering_info_updated_msg(len, key, len, value, len);
        } break;
        ...
        case EVENT_SENSOR_OPTICAL_FINGERPRINT_CMD_RESULT:
            ...
            on_fingerprint_cmd_msg(g_test_cali_result.cmd_id, g_test_cali_result.test_result.data, g_test_cali_result.result_length);
            break;
        case EVENT_SEND_DCS_MSG:
            egislog_i("EVENT_SEND_DCS_MSG value1:%d, value2:%d", value1, value2);
            on_send_dcsmsg_msg(event_id, buffer, buffer_size);
            break;
        case EVENT_SET_ACTION:
            g_dev->callback->set_action(g_dev->callback_context, value1, value2);
            break;
        default:
            break;
    };
...
int fingerprint_open(egis_fingerprint_hal_device_t **device)
{
    ...
    char cali_path[256] = "/data/vendor_de/0/fpdata\0";
    #ifdef EGIS_DBG
        egislog_i("DEBUG fingerprint_open enter");
    #else
        egislog_i("RELEASE fingerprint_open enter");
    #endif
