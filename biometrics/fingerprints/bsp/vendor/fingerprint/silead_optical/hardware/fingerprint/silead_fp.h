...
typedef struct silead_fp_handle {
    ...
    int32_t (*fp_test_get_image_info)(uint32_t *w, uint32_t *h, uint32_t *max_size, uint32_t *w_ori, uint32_t *h_ori, uint8_t *bitcount, uint8_t *bitcount_orig);
	...
	int32_t (*fp_cal_test_cmd)(uint32_t cmd, void *buffer, uint32_t *plen, uint32_t *result);
#endif
} silead_fp_handle_t;

#ifdef SL_FP_FEATURE_OPLUS_CUSTOMIZE
typedef enum _get_ta_info_type {
    TA_CMD_QUERY_ALGORITHM_PARAMETER = 1000,
    TA_CMD_QUERY_VERSION_INFO = 1001,
    TA_CMD_QUERY_FUNC_INFO = 1002,
    TA_CMD_QUERY_IMAGE_INFO = 1003,
    TA_CMD_MAX,
} get_ta_info_type_t;
#endif
