 ...
 * Bangxiong.Wu    2019/03/11   1.0.0      Add for enroll retry
 * Bangxiong.Wu    2019/06/21   1.0.1      modify for sync tpl to tee
 *
 *****************************************************************************/
...
int32_t silfp_cmd_test_get_image_info(uint32_t *w, uint32_t *h, uint32_t *max_size, uint32_t *w_ori, uint32_t *h_ori, uint8_t *bitcount, uint8_t *bitcount_orig)
{
    ...
    if (m_fp_impl_handler != NULL) {
        if (m_fp_impl_handler->fp_test_get_image_info != NULL) {
            ret = m_fp_impl_handler->fp_test_get_image_info(w, h, max_size, w_ori, h_ori, bitcount, bitcount_orig);
        } else {
...
int32_t sl_fp_query_algorithm_parameter(algotirhm_paramater_t *ptr)
{
    ...
    int32_t cmd_ta = 1000;
    uint32_t len = sizeof(algotirhm_paramater_t);
    uint32_t result = 0;

    if (m_fp_impl_handler != NULL) {
        if (m_fp_impl_handler->fp_cal_test_cmd != NULL) {
            ret = m_fp_impl_handler->fp_cal_test_cmd(cmd_ta, (uint8_t *)ptr, (uint32_t *)&len, &result);
        } else {
