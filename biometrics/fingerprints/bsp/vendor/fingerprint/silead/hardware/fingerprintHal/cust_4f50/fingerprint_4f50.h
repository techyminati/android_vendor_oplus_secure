...
//    int (*getEnrollmentTotalTimes)(struct fingerprint_device *dev);
    ...
    int (*getAlikeyStatus)(struct fingerprint_device *dev);

    int (*get_enrollment_total_times)(struct fingerprint_device *dev);
//    int (*cleanUp)(struct fingerprint_device *dev);
    int (*getBadPixels)(struct fingerprint_device *dev);
	
//    int (*setTouchEventListener)(struct fingerprint_device *dev);
    int (*setScreenState)(struct fingerprint_device *dev, uint32_t sreenstate);
...
//    int (*sendFingerprintCmd)(struct fingerprint_device *dev, int32_t cmd_id, int8_t* in_buf, uint32_t size);
    int (*sendFingerprintCmd)(struct fingerprint_device *dev, int32_t cmd_id, int8_t* in_buf);
    ...
    //int (*setFingerKeymode)(struct fingerprint_device *dev,uint32_t enable_keymode);
	//int (*getEngineeringInfo)(struct fingerprint_device *dev, uint32_t type);
