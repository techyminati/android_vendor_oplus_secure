...
typedef struct __attribute__((packed)) _tpls_v1 {
    T_TPL_MAP tpl[TPL_MAX_ST];
    int32_t  num;
    int32_t init_once;
    int32_t checksum;
...
static int64_t _storage_gen_id(int64_t old_value)
{
    int64_t id = -1;
    int32_t i = 0;
    ...
    srand(time(NULL));
    do {
        id = (((int64_t)rand()) << 32) | ((int64_t)rand());
        if ((id != 0) && (id != old_value)) {
            break;
        }
        i++;
    } while (i < 10);
...
static int32_t _storage_load_sets(T_TPL_SET *ptpls, const char *path, char *fname)
{
    int32_t ret = 0;
    FILE *fp = NULL;
    void *buf = NULL;
    T_TPL_SET_V1 *ptpls_v1 = NULL;
    int32_t checksum = 0;
    ...
    fseek(fp, 0, SEEK_SET);
    ret = fread(buf, sizeof(char), sizeof(T_TPL_SET), fp);
    fclose(fp);

    LOG_MSG_VERBOSE("read tpl sets size:%d new %d, old %d", ret, sizeof(T_TPL_SET), sizeof(T_TPL_SET_V1));

    if (ret == sizeof(T_TPL_SET)) {
        memcpy(ptpls, buf, sizeof(T_TPL_SET));
    } else if (ret == sizeof(T_TPL_SET_V1)) { // db upgrade
        ptpls_v1 = (T_TPL_SET_V1 *)buf;
        _storage_calc_checksum((char *)ptpls_v1, sizeof(T_TPL_SET_V1) - sizeof(int32_t), &checksum);
        if (checksum == ptpls_v1->checksum) {
            memcpy(ptpls->tpl, ptpls_v1->tpl, sizeof(ptpls->tpl));
            ptpls->num = ptpls_v1->num;
            ptpls->init_once = ptpls_v1->init_once;
            ptpls->db_id = _storage_gen_id(0);
            _storage_save_sets(path, TPL_FNAME, ptpls);
            LOG_MSG_DEBUG("tpl sets upgrade");

            ret = sizeof(T_TPL_SET);
        }
    }
...
static int32_t _storage_init()
{
    ...
    do {
        ...
        ret = _storage_load_sets(ptpls, path, fname);
        if (ret < 0) {
...
int32_t silfp_storage_save(const char *buf, const uint32_t len, int64_t sec_id, uint32_t *fid)
{
    ...
    if (ret >= 0) {
        ...
        if (i >= 0 && i < TPL_MAX_ST) {
            ...
            ptpls->db_id = _storage_gen_id(ptpls->db_id); // enroll new finger, upd db_id
