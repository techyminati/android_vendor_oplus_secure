...
static uint32_t m_key_mode_enable = 1;
...
{
    return (m_key_mode_enable && ((m_nav_type > NAV_TYPE_NONE) && (m_nav_type < NAV_TYPE_MAX))) ? 1 : 0;
}

static int32_t _nav_send_key_for_lite(uint32_t key)
{
                ...
                if (ret >= 0) {
                    if (IS_KEY_VALID(key)) {
                        ...
                        if (m_nav_type == NAV_TYPE_LITE) {
                            ret = _nav_send_key_for_lite(key);
                        } else {
...
{
    LOG_MSG_DEBUG("hycadd(%d --> %d)", m_key_mode_enable, enable);
    LOG_MSG_INFO("silfp_nav_set_key_mode (%d)", enable);

    if (m_key_mode_enable && !enable) {
        LOG_MSG_DEBUG("disable nav mode");
        m_key_mode_enable = enable;
        silfp_worker_screen_state_callback(silfp_worker_get_screen_state(), NULL);
    } else if (!m_key_mode_enable && enable) {
        LOG_MSG_DEBUG("enable nav mode");
        m_key_mode_enable = enable;
        silfp_worker_screen_state_callback(silfp_worker_get_screen_state(), NULL);
    }
}
