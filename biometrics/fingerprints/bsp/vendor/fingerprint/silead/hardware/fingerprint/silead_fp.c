...
static int32_t fp_tac_sync_ext_info(void)
{
    uint64_t times = 0;
    uint8_t data[8] = {0};
    int32_t i = 0;
    ...
    times = silfp_util_get_msec();
    for (i = 0; i < 8; i++) {
        data[i] = (uint8_t)((times >> (i*8)) & 0xFF);
    }

    return _tac_send_modified_command_1d_2r_and_get_always(TZ_FP_CMD_SEND_CMD_WITH_BUF_GET, data, len, REQ_CMD_SUB_ID_SYNC_EXT_INFO, &len, &result);
}
...
static int32_t fp_tac_open(const void *ta_name)
{
    ...
    ret = silfp_ca_open(ta_name);
    if (ret >= 0) {
        fp_tac_sync_ext_info();
    }
...
static int32_t fp_tac_set_enroll_challenge(uint64_t challenge)
{
    UNUSED(challenge);
...
static int32_t fp_tac_test_get_image_info(uint32_t *w, uint32_t *h, uint32_t *max_size, uint32_t *w_ori, uint32_t *h_ori, uint8_t *bitcount, uint8_t *bitcount_orig, uint8_t *type_16bit)
{
    int32_t ret = 0;
    uint32_t data1 = 0;
    uint32_t data2 = 0;
    ...
    if (ret >= 0) {
        ...
        if ((data1 & 0xFFFF0000) == 0 || (data2 & 0xFFFF0000) == 0) {
            ...
            if (max_size != NULL) {
                *max_size = (data3 & 0x000FFFFF);
            }
            if (type_16bit != NULL) {
                *type_16bit = ((data3 >> 20) & 0x0000000F);
            }
            if (bitcount != NULL) {
...
static int32_t fp_tac_check_broken(void)
{
    LOG_MSG_DEBUG("enter fp_tac_check_broken");

    return _tac_send_normal_command(TZ_FP_CMD_CHECK_BROKEN);
}
...
int32_t fp_tac_sync_ta_log(int32_t enable)
{
    return silfp_ca_sync_ta_log(enable);
}
