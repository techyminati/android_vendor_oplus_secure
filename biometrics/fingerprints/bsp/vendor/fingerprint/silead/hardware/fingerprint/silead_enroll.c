...
#define OPPO_ENROLL_TIME_OUT (10 * 60 * 1000)

static int32_t m_enroll_pause = 0;
static int64_t m_pre_enroll_time = 0; // seconds
...
uint64_t silfp_enroll_pre_enroll()
{
    ...
    LOG_MSG_INFO("challenge = %" PRIu64, challenge);

    m_enroll_pause = 0;
    m_pre_enroll_time = silfp_util_get_milliseconds();
...
int32_t silfp_enroll_enroll(const hw_auth_token_t *hat, uint32_t gid, uint32_t timeout_sec)
{
    ...
    LOG_MSG_INFO("enroll(gid=%u, timeout_sec=%u)", gid, timeout_sec);
    UNUSED(timeout_sec);

    if (hat == NULL) {
		LOG_MSG_INFO("hat == NULL");
        silfp_notify_send_error_notice(FINGERPRINT_ERROR_HW_UNAVAILABLE);
        return SL_SUCCESS;
    }

    LOG_MSG_INFO("hat(%" PRIu64 ":%" PRIu64 ":%" PRIu64 ")", hat->challenge, hat->user_id, hat->authenticator_id);
...
int32_t silfp_enroll_command(void)
{
    ...
    if (enroll_timeout_sec > 0) { // need check timeout
        LOG_MSG_DEBUG("check enroll timeout (%d)", OPPO_ENROLL_TIME_OUT);
        enroll_time = silfp_util_get_milliseconds();
        if (enroll_time > m_pre_enroll_time && enroll_time - m_pre_enroll_time >= OPPO_ENROLL_TIME_OUT) {
            silfp_notify_send_error_notice(FINGERPRINT_ERROR_TIMEOUT);
    ...
    if (ret >= 0) {
        do {
            ...
            if (ret >= 0) {
                // recheck finger status, if finger up, report move too fast
            ...
            if (ret < 0) {
                ...
                if (ret == -SL_ERROR_CANCELED) { // canceled
                    ...
                    case -SL_ERROR_EROLL_DUPLICATE: {
                        ...
                        silfp_notify_send_acquired_notice(notice_code);
                        break;
                    }
