...
#endif
#ifdef SECURITY_TYPE_BEANPOD
#include "beanpod/silead_beanpod_impl.h"
#endif
...
int32_t silfp_ca_send_modified_command(uint32_t cmd, void *buffer, uint32_t len, uint32_t flag, uint32_t v1, uint32_t v2, uint32_t *data1, uint32_t *data2)
{
    ...
    if (m_ta_send_error_times >= ERROR_TO_IDLE_STEPS) {
        return -SL_ERROR_TA_SEND_FAILED;
    }
...
#ifdef SECURITY_TYPE_BEANPOD
    if(ret < 0) {
        ret = silfp_ca_beanpod_register(&m_ca_handler, ta_name);
    }
#endif
#ifdef SECURITY_TYPE_NOSEC
    if (ret < 0) {
...
int32_t silfp_ca_sync_ta_log(int32_t enable)
{
    if (m_ca_handler.ca_sync_ta_log == NULL) {
        LOG_MSG_DEBUG("No implement ca_sync_ta_log");
        return 0;
    }

    return m_ca_handler.ca_sync_ta_log(enable);
}