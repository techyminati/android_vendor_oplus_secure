...
#define GP_BUF_MAX_EACH_SHM (1024*1020)
#ifdef SIL_FP_GP_MEM_SIZE_2M
#define GP_BUF_MAX_SIZE (2*GP_BUF_MAX_EACH_SHM)
#else
#define GP_BUF_MAX_SIZE (GP_BUF_MAX_EACH_SHM)
#endif

const char * __attribute__((weak)) silfp_ta_drv_get_ta_uuid(void);
int32_t __attribute__((weak)) silfp_ta_drv_open(void);
int32_t __attribute__((weak)) silfp_ta_drv_close(void);

int32_t __attribute__((weak)) silfp_mtee_ca_open(void);
int32_t __attribute__((weak)) silfp_mtee_ca_close(void);
int32_t __attribute__((weak)) silfp_mtee_ca_send_normal_command(uint32_t cmd, uint32_t v1, uint32_t v2, int32_t status);
        ...
        uint32_t *data1, uint32_t *data2)
{
    ...
    uint32_t mem_type = TEEC_MEMREF_TEMP_INPUT;
    ...
    if (flag != 0) {
        mem_type = TEEC_MEMREF_TEMP_INOUT;
    } else {
        mem_type = TEEC_MEMREF_TEMP_INPUT;
    }

    sOperation.params[0].value.a = v1;
    sOperation.params[0].value.b = v2;

    if (len > GP_BUF_MAX_SIZE) {
        LOG_MSG_ERROR("buf is too larger (%d but %d), not support!", GP_BUF_MAX_SIZE, len);
        return -SL_ERROR_BAD_PARAMS;
    } else if (len <= GP_BUF_MAX_EACH_SHM) {
        sOperation.params[2].tmpref.buffer = buffer;
        sOperation.params[2].tmpref.size = len;
        sOperation.paramTypes = TEEC_PARAM_TYPES(TEEC_VALUE_INOUT, TEEC_VALUE_OUTPUT, mem_type, TEEC_NONE);
    } else {
        sOperation.params[2].tmpref.buffer = buf;
        sOperation.params[2].tmpref.size = GP_BUF_MAX_EACH_SHM;
        sOperation.params[3].tmpref.buffer = (buf + sOperation.params[2].tmpref.size);
        sOperation.params[3].tmpref.size = len - GP_BUF_MAX_EACH_SHM;
        sOperation.paramTypes = TEEC_PARAM_TYPES(TEEC_VALUE_INOUT, TEEC_VALUE_OUTPUT, mem_type, mem_type);
    }
...
static int32_t _ca_send_normal_commond_with_check(uint32_t cmd, uint32_t v1, uint32_t v2, uint32_t v3, uint32_t v4, uint32_t *data1, uint32_t *data2, uint32_t *data3)
{
    int32_t ret = _ca_send_normal_command(cmd, v1, v2, v3, v4, data1, data2, data3);
    if (ret >= 0 && silfp_mtee_ca_send_normal_command) {
        ret = silfp_mtee_ca_send_normal_command(cmd, v1, v2, ret);
    }
    return ret;
}
...
static int32_t _ca_open(const void *ta_name)
{
    ...
    if (ret < 0 && silfp_ta_drv_get_ta_uuid) {
        ta_name = silfp_ta_drv_get_ta_uuid();
        ret = _ca_str_to_uuid(ta_name, &uuid);
        if (ret >= 0) {
            ret = m_tz_handle->TEECCom_load_trustlet(m_tz_handle, &m_fp_context, &m_fp_handle, FP_TZAPP_PATH, &uuid);
        }
    ...
    if (ret >= 0 && silfp_mtee_ca_open) {
        silfp_mtee_ca_open();
    }

    if (ret >= 0 && silfp_ta_drv_open) {
        silfp_ta_drv_open();
    }
...
static int32_t _ca_close(void)
{
    ...
    if (silfp_mtee_ca_close) {
        silfp_mtee_ca_close();
    }

    if (silfp_ta_drv_close) {
        silfp_ta_drv_close();
    }
...
int32_t silfp_ca_gp_register(ca_impl_handle_t *handle, const void *ta_name)
{
    ...
    handle->ca_send_normal_command = _ca_send_normal_commond_with_check;
