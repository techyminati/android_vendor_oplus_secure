...
#endif
#ifdef JIIOV_OPTICAL_FP_ENABLE
using vendor::oplus::hardware::biometrics::fingerprint::V2_1::implementation::AncFingerprint;
#endif
...
int main() {
    ...
    int retry_times = 0;
    int status = 0;
    uint32_t ret = oplus_fp_type_init();
    if (0 != ret) {
        ALOGE("fp type init failed");
        return -1;
    }
    ...
    if (FP_GOODIX == fp_config_info_init.fp_factory_type) {
        ...
        retry_times = RETRY_TOTAL_TIMES;
        while (retry_times) {
            if (bio->init()) {
                retry_times--;
                sleep(1);
                ALOGE("fail to init GF HAL, retry_times = %d", retry_times);
            } else {
                break;
            }
        }

        if (retry_times > 0 && bio != nullptr) {
            android::status_t status = bio->registerAsService();
            if (status != android::OK) {
                ALOGE("Cannot register GF HAL service");
                return -1;
            } else {
                ALOGD("register GF HAL service ok");
            }
        } else {
...
#ifdef FPC_DDR_SAME_POWER_SUPPLY
                    if ((status == 99) && (0 == strcmp("F_1541", fp_config_info_init.fp_id_string))) {
                        err = hal->registerAsService("default");
                        if (err != OK) {
                            ALOGE("Cannot register fingerprint HAL service error=%d", err);
                            return -1;
                        }
                        ALOGD("Add fp to ServiceManager ok, just for Homer F_1541 project");
                    }
#endif
                    goto out;
                }
...
#ifdef SILEAD_CAPACITIVE_FP_ENABLE
    } else if (FP_SILEAD == fp_config_info_init.fp_factory_type) {
        ALOGE("silead capacitive fingerprint");
        property_set(OPTICAL_FEATURE, "0");
        ...
        ALOGD("Add silead capacitive to ServiceManager ok");

#endif
    } else if (FP_JIIOV == fp_config_info_init.fp_factory_type) {
        ALOGE("ANC  fingerprint");

#ifdef OPLUS_JIIOV_SUPPORT
        property_set(OPTICAL_FEATURE, "1");
        set_jiiov_icon_property();
#ifdef FP_CONFIG_SETTINGS_ENABLE
        fpSettingsInit();
#endif

        configureRpcThreadpool(1, false /*callerWillJoin*/);
        android::sp<AncFingerprint> bio =
        static_cast<AncFingerprint *>(AncFingerprint::getInstance());
        int retry = 3;
        ...
        while (retry) {
            if (!bio->init()) {
                retry--;
                sleep(1);
                ALOGE("fail to init ANC HAL, retry = %d", retry);
            } else {
                break;
            }
        }

        if (retry > 0 && bio != nullptr) {
            android::status_t status = bio->registerAsService();
            if (status != android::OK) {
                ALOGE("Cannot register ANC HAL service");
                return -1;
            } else {
                ALOGD("register ANC HAL service ok");
            }
        } else {
            ALOGE("Can't create instance of ANC, nullptr");
            return -1;
        }
        ALOGD("Add ANC to ServiceManager ok");
        ALOGE("ANC HAL is ready");
        // set_jiiov_icon_property();
#endif
    } else {
