...
ifeq ($(FP_SILEAD_SUPPORT), y)
LOCAL_CFLAGS     += -DSILEAD_CAPACITIVE_FP_ENABLE
LOCAL_SRC_FILES +=  silead_optical/SileadFingerprint.cpp
LOCAL_C_INCLUDES += $(LOCAL_PATH)/../silead/hardware/fingerprint/
endif
...
#add for jiiov fp
ifeq ($(FP_JIIOV_OPTICAL_SUPPORT), y)
LOCAL_CFLAGS += -DOPLUS_JIIOV_SUPPORT \
                -DJIIOV_OPTICAL_FP_ENABLE
LOCAL_SHARED_LIBRARIES += anc.hal
LOCAL_C_INCLUDES += $(LOCAL_PATH)/jiiov_optical/anc_hal_lib/inc
LOCAL_C_INCLUDES += $(LOCAL_PATH)/jiiov_optical/
LOCAL_SRC_FILES +=  jiiov_optical/anc_fingerprint.cpp
endif
...
LOCAL_C_INCLUDES += $(LOCAL_PATH)/../egistec/egis_biometrics \
					$(LOCAL_PATH)/../egistec/common/platform/inc \
					$(LOCAL_PATH)/../egistec/flow/ \
					$(LOCAL_PATH)/../egistec/flow/navi/include	\
					$(LOCAL_PATH)/../egistec/flow/external_include 	\
					$(LOCAL_PATH)/../egistec/flow/def 			\
					$(LOCAL_PATH)/../egistec/flow/core/include		\
					$(LOCAL_PATH)/../egistec/flow/util/include 		\
					$(LOCAL_PATH)/../egistec/flow/plugin/sdk_save_image/inc \
					$(LOCAL_PATH)/../egistec/flow/transporter/include \
					$(LOCAL_PATH)/../hwbinder/egis/include \
					$(LOCAL_PATH)/../egistec/common/platform/inc \
					$(LOCAL_PATH)/../egistec/common/definition  \
					$(LOCAL_PATH)/../egistec/common/utility/inih \
					$(LOCAL_PATH)/../egistec/common/utility/config/inc \
					$(LOCAL_PATH)/../egistec/common/utility/hash_map
					
LOCAL_SRC_FILES += ./egis/egis_hidl.cpp \
				   ../egistec/egis_biometrics/egis_fingerprint.c \
				   ../egistec/egis_biometrics/qsee_keymaster.c \
				   ../egistec/common/platform/src/linux/plat_log_linux.c
else
LOCAL_CFLAGS    += -DEGIS_CAPACITY_FP_ENABLE
...
LOCAL_SRC_FILES +=  ../egistec/egis_biometrics/egis_fingerprint.c \
		      egis/egis_hidl.cpp
endif
endif
...
## Add by Mingfu.Xie@Secure.Biometrics.Fingerprint For Performance HIDL, 20210917
LOCAL_CFLAGS += -DFP_SET_UXTREAD
LOCAL_C_INCLUDES += vendor/oplus/hardware/performance/1.0
LOCAL_SHARED_LIBRARIES += vendor.oplus.hardware.performance@1.0
ifeq ($(FP_CONFIG_UXTHREAD_HIGHT_LEVEL),yes)
LOCAL_CFLAGS += -DFP_UXTHREAD_HIGHT_LEVEL
endif
endif
