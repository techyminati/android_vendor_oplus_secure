...
**
** Version: V1.0
** Date : 11:03:26, 2017-10-21
** Author: Kui.Jiang@PSW.Android.FaceRecognition, Add for face recognition
**
** ---------------------------Revision History--------------------------------
**  <author>    <data>     <desc>
**
************************************************************************************/
#define LOG_TAG "[FP_HAL][Handler]"
...
Handler::Handler(const char *name, const sp<HandlerCallback> &callback)
    ...
    status_t result = mHandlerThread->run(name, PRIORITY_URGENT_DISPLAY);  //--do the threadLoop
    LOG_D(LOG_TAG, "mHandlerThread run, name = %s result = %d", name, result);
    if (result) {
        LOG_E(LOG_TAG, "Could not start HandlerThread(%s) thread due to error %d.", name, result);
    }
    LOG_D(LOG_TAG, "Handler create success");
}

Handler::~Handler() {
    LOG_D(LOG_TAG, "HandlerThread destroy");
    mHandlerThread = NULL;
}
...
void Handler::dispatchOnce() {
    ...
    {  // acquire lock
        ...
        // LOG_D(LOG_TAG, "dispatchOnce getLock enter, wakeSource = %s, count = %d",
        // mWakeSourceName.string(), count);
        Vector<FpMessage>::iterator it;
        int64_t                     now = uptimeMillis();
        for (it = mMessageQueue.begin(); it != mMessageQueue.end();) {
            if (now >= (*it).when) {
                mHandlerCallback->handleMessage(*it); //callback handleMessage
                it = mMessageQueue.erase(it);
            } else {
                if (nextPollTimeoutMillis == -1) {
                    nextPollTimeoutMillis = (*it).when - now;
                } else {
                    nextPollTimeoutMillis = (((*it).when - now) < nextPollTimeoutMillis)
                        ? ((*it).when - now)
...
int Handler::findMessage(int what) {
    size_t count = mMessageQueue.size();
    // LOG_D(LOG_TAG, "findMessage, size = %d, what = %d", count, what);
...
void Handler::sendMessage(int what) { sendMessageDelayed(what, 0); }

void Handler::sendMessage(FpMessage msg) { sendMessageDelayed(msg, 0); }

void Handler::sendMessageDelayed(int what, long delayMillis) {
    {  // acquire lock
        AutoMutex _l(mLock);
        // LOG_D(LOG_TAG, "sendMessageDelayed, what = %d, size = %d, delayMillis = %ld",
        // what, mMessageQueue.size(), delayMillis);
        FpMessage msg(what);
...
void Handler::sendMessageDelayed(FpMessage msg, long delayMillis) {
    {  // acquire lock
        AutoMutex _l(mLock);
        // LOG_D(LOG_TAG, "sendMessageDelayed, size = %d, what = %d, delayMillis = %ld",
        // mMessageQueue.size(), msg.what, delayMillis);
        msg.when     = uptimeMillis() + delayMillis;
        size_t count = mMessageQueue.size();
        if (count <= 0) {
            mMessageQueue.push_back(msg);
...
HandlerThread::HandlerThread(const sp<Handler> &handler)
    ...
    LOG_D(LOG_TAG, "HandlerThread create");
}
