 ...
 ************************************************************************************/
#define LOG_TAG "[FP_HAL][BiometricsFingerprint]"
...
using RequestStatus = vendor::oplus::hardware::biometrics::fingerprint::V2_1::RequestStatus;
...
Return<RequestStatus> BiometricsFingerprint::setActiveGroup(
    ...
    const char* setPath = storePath.c_str();
    sprintf(checkPath, "/data/vendor_de/%u/fpdata", gid);
    ...
    if (setPath[storePath.size() - 1] == '/') {
        checkPath[strlen(checkPath)] = '/';
    }
    if (strcmp(setPath, checkPath)) {
        LOG_E(LOG_TAG, "storePath is not correct");
        return RequestStatus::SYS_EINVAL;
    }
...
Return<RequestStatus> BiometricsFingerprint::authenticate(uint64_t operationId, uint32_t gid) {
    LOG_I(LOG_TAG, "enter to [%s] operationId = %lu, gid =%d", __func__, operationId, gid);
    return authenticateAsType(operationId, gid, FingerprintAuthType::TYPE_KEYGUARD);
}

//add by vendor
Return<RequestStatus> BiometricsFingerprint::authenticateAsType(
    uint64_t operationId, uint32_t gid, FingerprintAuthType authtype) {
    LOG_I(LOG_TAG, "enter to [%s]", __func__);
    action_message_t action;
    memset(&action, 0, sizeof(action_message_t));
    ...
    action.data.authenticate.authtype    = static_cast<uint32_t>(authtype);
    mHalContext->mFingerprintManager->sendMessageToWorkThread(action);
    return RequestStatus::SYS_OK;
}
...
Return<RequestStatus> BiometricsFingerprint::setScreenState(FingerprintScreenState screen_state) {
...
Return<RequestStatus> BiometricsFingerprint::sendFingerprintCmd(
...
void BiometricsFingerprint::notify(const fingerprint_msg_t* msg) {
    ...
    switch (msg->type) {
        ...
        case ENGINEERING_INFO: {
            ...
            if (!thisPtr->mClientCallback->onEngineeringInfoUpdated(len,
        ...
        case FINGERPRINT_TEMPLATE_ENUMERATING: {
            ...
            if (!thisPtr->mClientCallback->onSyncTemplates(devId,
                hidl_vec<uint32_t>(hidl_fids), msg->data.enumerated.gid).isOk()) {
                LOG_E(LOG_TAG, "failed to invoke fingerprint onEnumerate callback");
            }
            free(fids);
        } break;
        ...
        case FINGERPRINT_OPTICAL_SENDCMD: {
            ...
            if (!thisPtr->mClientCallback
                    ->onFingerprintCmd(msg->data.test.cmd_id, hidl_vec<int8_t>(test_result),
